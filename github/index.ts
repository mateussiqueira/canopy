...
import { createOpencodeClient } from "@canopystack/sdk"
...
function useShareUrl() {
  return isMock() ? "https://dev.canopy.dev" : "https://canopy.dev"
}
...
async function getAccessToken() {
  ...
  let response
  if (isMock()) {
    response = await fetch("https://api.canopy.dev/exchange_github_app_token_with_pat", {
      method: "POST",
      headers: {
    ...
    response = await fetch("https://api.canopy.dev/exchange_github_app_token", {
      method: "POST",
      headers: {
