...
# パッケージマネージャー
npm i -g canopy@latest        # bun/pnpm/yarn でもOK
scoop install canopy             # Windows
choco install canopy             # Windows
brew install mateussiqueira/tap/canopy # macOS と Linux（推奨。常に最新）
brew install canopy              # macOS と Linux（公式 brew formula。更新頻度は低め）
sudo pacman -S canopy            # Arch Linux (Stable)
paru -S canopy-bin               # Arch Linux (Latest from AUR)
mise use -g canopy               # どのOSでも
nix run nixpkgs#canopy           # または github:mateussiqueira/canopy で最新 dev ブランチ
```
...
Canopy はデスクトップアプリとしても利用できます。[releases page](https://github.com/mateussiqueira/canopy/releases) から直接ダウンロードするか、[canopy.dev/download](https://canopy.dev/download) を利用してください。
...
1. `$CANOPY_INSTALL_DIR` - カスタムのインストールディレクトリ
2. `$XDG_BIN_DIR` - XDG Base Directory Specification に準拠したパス
3. `$HOME/bin` - 標準のユーザー用バイナリディレクトリ（存在する場合、または作成できる場合）
4. `$HOME/.canopy/bin` - デフォルトのフォールバック
...
Canopy には組み込みの Agent が2つあり、`Tab` キーで切り替えられます。
...
[agents](https://canopy.dev/docs/agents) の詳細はこちら。

### ドキュメント

Canopy の設定については [**ドキュメント**](https://canopy.dev/docs) を参照してください。

### コントリビュート

Canopy に貢献したい場合は、Pull Request を送る前に [contributing docs](./CONTRIBUTING.md) を読んでください。

### Canopy の上に構築する

Canopy に関連するプロジェクトで、名前に "canopy"（例: "canopy-dashboard" や "canopy-mobile"）を含める場合は、そのプロジェクトが Canopy チームによって作られたものではなく、いかなる形でも関係がないことを README に明記してください。

---

**コミュニティに参加** [Discord](https://discord.gg/canopy) | [X.com](https://x.com/canopy)
