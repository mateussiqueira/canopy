...
npm i -g canopy@latest        # o bun/pnpm/yarn
scoop install canopy             # Windows
choco install canopy             # Windows
brew install mateussiqueira/tap/canopy # macOS y Linux (recomendado, siempre al día)
brew install canopy              # macOS y Linux (fórmula oficial de brew, se actualiza menos)
sudo pacman -S canopy            # Arch Linux (Stable)
paru -S canopy-bin               # Arch Linux (Latest from AUR)
mise use -g canopy               # cualquier sistema
nix run nixpkgs#canopy           # o github:mateussiqueira/canopy para la rama dev más reciente
```
...
Canopy también está disponible como aplicación de escritorio. Descárgala directamente desde la [página de releases](https://github.com/mateussiqueira/canopy/releases) o desde [canopy.dev/download](https://canopy.dev/download).
...
1. `$CANOPY_INSTALL_DIR` - Directorio de instalación personalizado
2. `$XDG_BIN_DIR` - Ruta compatible con la especificación XDG Base Directory
3. `$HOME/bin` - Directorio binario estándar del usuario (si existe o se puede crear)
4. `$HOME/.canopy/bin` - Alternativa por defecto
...
Canopy incluye dos agentes integrados que puedes alternar con la tecla `Tab`.
...
Más información sobre [agentes](https://canopy.dev/docs/agents).

### Documentación

Para más información sobre cómo configurar Canopy, [**ve a nuestra documentación**](https://canopy.dev/docs).

### Contribuir

Si te interesa contribuir a Canopy, lee nuestras [docs de contribución](./CONTRIBUTING.md) antes de enviar un pull request.

### Proyectos basados en Canopy

Si estás trabajando en un proyecto basado en Canopy y usas "canopy" como parte del nombre, por ejemplo, "canopy-dashboard" u "canopy-mobile", agrega una nota en tu README para aclarar que no está hecho por el equipo de Canopy y que no está afiliado con nosotros de ninguna manera.

---

**Únete a nuestra comunidad** [Discord](https://discord.gg/canopy) | [X.com](https://x.com/canopy)
