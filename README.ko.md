...
# 패키지 매니저
npm i -g canopy@latest        # bun/pnpm/yarn 도 가능
scoop install canopy             # Windows
choco install canopy             # Windows
brew install mateussiqueira/tap/canopy # macOS 및 Linux (권장, 항상 최신)
brew install canopy              # macOS 및 Linux (공식 brew formula, 업데이트 빈도 낮음)
sudo pacman -S canopy            # Arch Linux (Stable)
paru -S canopy-bin               # Arch Linux (Latest from AUR)
mise use -g canopy               # 어떤 OS든
nix run nixpkgs#canopy           # 또는 github:mateussiqueira/canopy 로 최신 dev 브랜치
```
...
Canopy 는 데스크톱 앱으로도 제공됩니다. [releases page](https://github.com/mateussiqueira/canopy/releases) 에서 직접 다운로드하거나 [canopy.dev/download](https://canopy.dev/download) 를 이용하세요.
...
Canopy 에는 내장 에이전트 2개가 있으며 `Tab` 키로 전환할 수 있습니다.
...
[agents](https://canopy.dev/docs/agents) 에 대해 더 알아보세요.

### 문서

Canopy 설정에 대한 자세한 내용은 [**문서**](https://canopy.dev/docs) 를 참고하세요.

### 기여하기

Canopy 에 기여하고 싶다면, Pull Request 를 제출하기 전에 [contributing docs](./CONTRIBUTING.md) 를 읽어주세요.

### Canopy 기반으로 만들기

Canopy 와 관련된 프로젝트를 진행하면서 이름에 "canopy"(예: "canopy-dashboard" 또는 "canopy-mobile") 를 포함한다면, README 에 해당 프로젝트가 Canopy 팀이 만든 것이 아니며 어떤 방식으로도 우리와 제휴되어 있지 않다는 점을 명시해 주세요.

---

**커뮤니티에 참여하기** [Discord](https://discord.gg/canopy) | [X.com](https://x.com/canopy)
