...
import { ConfigProvider } from "@canopystack/core/config/provider"
import { ConfigMigrateV1 } from "@canopystack/core/v1/config/migrate"
...
import { Policy } from "@canopystack/core/policy"
...
describe("Config", () => {
  ...
  it.live("loads global, ancestor, and .canopy configuration up to the project boundary", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.mkdir(path.join(root, ".canopy"), { recursive: true })
            await fs.mkdir(path.join(directory, ".canopy"), { recursive: true })
            await Promise.all([
              ...
              fs.writeFile(path.join(root, ".canopy", "opencode.json"), JSON.stringify({ $schema: "root-dot" })),
              fs.writeFile(
                path.join(directory, ".canopy", "opencode.jsonc"),
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(entries.filter((entry) => entry.type === "directory").map((entry) => entry.path)).toEqual([
              ...
              AbsolutePath.make(path.join(root, ".canopy")),
              AbsolutePath.make(path.join(directory, ".canopy")),
            ])
