...
import { FrontmatterError } from "@canopystack/core/v1/config/error"
import { ConfigMarkdown } from "@/config/markdown"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Glob } from "@canopystack/core/util/glob"
...
const CANOPY_SKILL_PATTERN = "{skill,skills}/**/SKILL.md"
...
const CUSTOMIZE_CANOPY_SKILL_NAME = "customize-opencode"
const CUSTOMIZE_CANOPY_SKILL_DESCRIPTION =
  "Use ONLY when the user is editing or creating opencode's own configuration: opencode.json, opencode.jsonc, files under .canopy/, or files under ~/.config/opencode/. Also use when creating or fixing opencode agents, subagents, skills, plugins, MCP servers, or permission rules. Do not use for the user's own application code, or for any project that is not configuring opencode itself."
const CUSTOMIZE_CANOPY_SKILL_BODY = SkillPlugin.CustomizeOpencodeContent
...
const discoverSkills = Effect.fnUntraced(function* (
  ...
  for (const dir of configDirs) {
    yield* scan(state, dir, CANOPY_SKILL_PATTERN)
  }
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make(
      Effect.fn("Skill.state")(function* () {
        ...
        s.skills[CUSTOMIZE_CANOPY_SKILL_NAME] = {
          name: CUSTOMIZE_CANOPY_SKILL_NAME,
          description: CUSTOMIZE_CANOPY_SKILL_DESCRIPTION,
          location: "<built-in>",
          content: CUSTOMIZE_CANOPY_SKILL_BODY,
        }
