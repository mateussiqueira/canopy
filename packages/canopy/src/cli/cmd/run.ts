import type { PermissionV1 } from "@canopystack/core/v1/permission"
...
//      Canopy server and runs interactive mode against it.
//
...
import { createOpencodeClient, type OpencodeClient, type ToolPart } from "@canopystack/sdk/v2"
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run Canopy with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running Canopy server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to CANOPY_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to CANOPY_SERVER_USERNAME or 'canopy')",
      })
      .option("dir", {
        type: "string",
