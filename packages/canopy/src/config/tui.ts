...
import { isRecord } from "@canopystack/tui/util/record"
...
import { TuiKeybind } from "@canopystack/tui/config/keybind"
...
import { TuiConfig } from "@canopystack/tui/config"
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  // Every config dir we may read from: global config dir, any `.canopy`
  // folders between cwd and home, and CANOPY_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.CANOPY_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit CANOPY_TUI_CONFIG override, if set.
  if (Flag.CANOPY_TUI_CONFIG) {
    const configFile = Flag.CANOPY_TUI_CONFIG
  ...
  // 4. `.canopy` directories (and CANOPY_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".canopy") || dir === Flag.CANOPY_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".canopy") && dir !== Flag.CANOPY_CONFIG_DIR) continue
