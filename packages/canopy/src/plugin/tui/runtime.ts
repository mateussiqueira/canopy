...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@canopystack/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { errorData, errorMessage } from "@canopystack/tui/util/error"
...
import { hasTheme, upsertTheme } from "@canopystack/tui/context/theme"
import { Global } from "@canopystack/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@canopystack/core/util/flock"
import { Flag } from "@canopystack/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@canopystack/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@canopystack/core/v1/config/plugin"
import { createCommandShim } from "@canopystack/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@canopystack/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".canopy"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".canopy", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".canopy", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.CANOPY_PURE ? [] : pluginOrigins
    if (Flag.CANOPY_PURE && pluginOrigins.length) {
    }
