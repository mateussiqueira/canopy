...
import { createOpencodeClient } from "@canopystack/sdk/v2"
...
function serverFetch(
  ...
  return HttpServer.HttpServer.use((server) =>
    Effect.sync(() => {
      void serverPath
      Flag.CANOPY_SERVER_PASSWORD = input?.password
      Flag.CANOPY_SERVER_USERNAME = input?.username
...
function writeProjectSkill(dir: string) {
  return FSUtil.Service.use((fs) =>
    fs.writeWithDirs(
      path.join(dir, ".canopy", "skills", "project-rest-skill", "SKILL.md"),
      `---
