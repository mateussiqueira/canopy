...
import { EventSequenceTable } from "@canopystack/core/event/sql"
...
import { WorkspaceTable } from "@canopystack/core/control-plane/workspace.sql"
...
const originalEnv = {
  CANOPY_AUTH_CONTENT: process.env.CANOPY_AUTH_CONTENT,
  CANOPY_EXPERIMENTAL_WORKSPACES: process.env.CANOPY_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.CANOPY_EXPERIMENTAL_WORKSPACES = "true"
})
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.CANOPY_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        ...
        expect(JSON.parse(recorded.calls.create[0].env.CANOPY_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.CANOPY_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.CANOPY_EXPERIMENTAL_WORKSPACES).toBe("true")
