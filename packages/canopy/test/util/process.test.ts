...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.CANOPY_TEST ?? "")'), {
      env: {
        CANOPY_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "CANOPY_TEST_SHELL"], {
      shell: true,
      env: {
        CANOPY_TEST_SHELL: "ok",
      },
    })

    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("CANOPY_TEST_SHELL=ok")
  })
