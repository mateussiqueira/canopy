...
- `config?: Partial<Config.Info>` - Write an `canopy.json` config file
...
- Directories are created in the system temp folder with prefix `canopy-test-`
...
it.instance("uses the temp directory", () =>
  Effect.gen(function* () {
    ...
    expect(test.directory).toContain("canopy-test-")
  }),
)
```
