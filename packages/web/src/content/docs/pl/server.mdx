---
title: Serwer
description: Interakcja z serwerem Canopy za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `canopy serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient Canopy.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `Canopy.local` |
...
Ustaw `OPENCODE_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `Canopy` lub ustaw `OPENCODE_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `canopy serve`, jak i `canopy web`.
...
Kiedy uruchomisz `Canopy`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera Canopy do programowej interakcji z Canopy.
:::

Ta architektura umożliwia obsługę wielu klientów przez Canopy i programową interakcję z kodem otwartym.

Możesz uruchomić `canopy serve`, aby uruchomić samodzielny serwer. Jeśli masz
Canopy TUI działa, `canopy serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki Canopy [IDE](/docs/ide).
...
Serwer Canopy udostępnia następujące interfejsy API.
