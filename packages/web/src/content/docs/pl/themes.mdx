...
Dzięki Canopy możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie Canopy używa naszego własnego motywu `Canopy`.
...
Canopy ma kilka wbudowanych motywów.
...
- Chcą, aby Canopy pasował do wyglądu ich terminala
...
Canopy obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/Canopy/themes/*.json` lub `$XDG_CONFIG_HOME/Canopy/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.Canopy/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.Canopy/themes/*.json`
