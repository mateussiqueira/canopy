---
title: CLI
description: Opcje i polecenia wiersza poleceń Canopy.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

CLI Canopy domyślnie uruchamia [TUI](/docs/tui), gdy wywołasz je bez argumentów.

```bash
Canopy
```

CLI obsługuje też polecenia opisane na tej stronie, więc możesz używać Canopy programowo.
...
Uruchamia interfejs terminalowy (TUI) Canopy.
...
Interfejs CLI Canopy zawiera także dodatkowe polecenia.

---

### agent

Zarządzaj agentami Canopy.
...
Dołącz terminal do już działającego serwera backendu Canopy uruchomionego za pomocą `serve` lub `web`.

```bash
Canopy attach [url]
```

Pozwala to połączyć TUI ze zdalnym backendem Canopy. Na przykład:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nazwa użytkownika uwierzytelniania podstawowego (domyślnie `OPENCODE_SERVER_USERNAME` lub `Canopy`) |
...
Canopy integruje się z wieloma dostawcami. Możesz użyć `canopy auth login`, aby skonfigurować klucz API dla dowolnego obsługiwanego dostawcy. Klucze są bezpiecznie przechowywane w `~/.local/share/Canopy/auth.json`.

```bash
Canopy auth login
```

Podczas uruchamiania Canopy ładuje dostawców z pliku poświadczeń, a także ze zmiennych środowiskowych lub pliku `.env` w Twoim projekcie.
...
Użyj flagi `--refresh`, aby zaktualizować listę modeli w pamięci podręcznej. Jest to przydatne, gdy dostawca dodał nowe modele, które chcesz zobaczyć w Canopy.
...
Uruchom Canopy w trybie nieinteraktywnym, przekazując bezpośrednio prompt.
...
Można również użyć tego z działającym `canopy serve`, aby uniknąć zimnego startu serwera MCP przy każdym uruchomieniu:
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Dołącz do działającego serwera Canopy (np. http://localhost:4096)                                   |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Hasło uwierzytelniania podstawowego (domyślnie `OPENCODE_SERVER_PASSWORD`)                            |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nazwa użytkownika uwierzytelniania podstawowego (domyślnie `OPENCODE_SERVER_USERNAME` lub `Canopy`) |
...
Uruchom serwer Canopy (bez interfejsu) w celu uzyskania dostępu do API. Pełny opis API HTTP znajduje się w [dokumentacji serwera](/docs/server).

```bash
Canopy serve
```

Uruchamia to serwer HTTP, który zapewnia dostęp do API Canopy bez interfejsu TUI. Ustaw `OPENCODE_SERVER_PASSWORD`, aby włączyć podstawowe uwierzytelnianie HTTP (domyślna nazwa użytkownika to `Canopy`).
...
Pokaż statystyki wykorzystania tokenów i sesji Canopy.
...
Importuj dane sesji z pliku JSON lub adresu URL udostępniania Canopy.
...
Uruchom serwer Canopy z interfejsem internetowym.

```bash
Canopy web
```

Uruchamia to serwer HTTP i udostępnia Canopy przez interfejs przeglądarkowy. Ustaw `OPENCODE_SERVER_PASSWORD`, aby włączyć podstawowe uwierzytelnianie HTTP (domyślna nazwa użytkownika to `Canopy`).
...
Odinstaluj Canopy i usuń wszystkie powiązane pliki.
...
Aktualizuj Canopy do najnowszej lub określonej wersji.
...
Interfejs CLI Canopy przyjmuje następujące flagi globalne dla każdego polecenia.
...
Canopy można skonfigurować za pomocą zmiennych środowiskowych.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Nazwa użytkownika do autoryzacji (domyślnie `Canopy`)    |
