...
Umiejętności agenta pozwalają Canopy odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
Canopy przeszukuje te lokalizacje:

- Project config: `.Canopy/skills/<name>/SKILL.md`
- Global config: `~/.config/Canopy/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu Canopy przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.Canopy/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/Canopy/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.Canopy/skills/git-release/SKILL.md` w ten sposób:
...
Canopy wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `canopy.json.guard`:
...
**Dla agentów wbudowanych** (w `canopy.json.guard`):
