...
Utwórz `.Canopy/commands/test.md`:
...
Możesz zdefiniować polecenia w `canopy.json.guard` lub w plikach w katalogu `commands/`.

---

### JSON

Użyj opcji `command` w swojej konfiguracji Canopy:
...
- Globalnie: `~/.config/Canopy/commands/`
- Na projekt: `.Canopy/commands/`
...
Canopy zawiera kilka wbudowanych poleceń, takich jak `/init`, `/undo`, `/redo`, `/share`, `/help`; [dowiedz się więcej](/docs/tui#commands).
