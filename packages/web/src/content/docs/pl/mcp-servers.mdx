...
Możesz dodać dodatkowe informacje do Canopy za pomocą _Model Context Protocol_, w skrócie MCP. Canopy obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [Canopy Config](https://canopy.dev/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/Canopy`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
Canopy automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, Canopy:
...
Jeśli serwer wymaga uwierzytelnienia, Canopy poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `canopy mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji Canopy bezpieczne przechowa tokeny w `~/.local/share/Canopy/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w Canopy, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie Canopy, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć Canopy z kontem Sentry.
