---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w Canopy.
---

import config from "../../../../config.mjs"
export const console = config.console

Canopy używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji Canopy.
...
w `~/.local/share/Canopy/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim Canopy
config.
...
Canopy Zen to lista modeli dostarczonych przez zespół Canopy, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z Canopy. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od Canopy Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `Canopy Zen` i przejdź do [canopy.dev/auth](https://canopy.dev/zen).
...
Działa jak każdy inny dostawca w Canopy i jest całkowicie opcjonalny w użyciu.

---

## Canopy Go

Canopy Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół Canopy, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z Canopy.

1. Uruchom polecenie `/connect` w TUI, wybierz `Canopy Go` i przejdź do [canopy.dev/auth](https://canopy.dev/zen).
...
Aby używać Amazon Bedrock z Canopy:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania Canopy:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `canopy.json.guard`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w Canopy nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować Canopy do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod Canopy działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji Canopy.
...
Canopy używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować Canopy
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`canopy.json.guard` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [Canopy-gitlab-auth](https://www.npmjs.com/package/Canopy-gitlab-auth).

##### Konfiguracja

Customize through `canopy.json.guard`:
...
Aby korzystać z subskrypcji GitHub Copilot z kodem Canopy:
...
Aby używać Google Vertex AI z Canopy:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania Canopy.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą Canopy, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [Canopy-helicone-session](https://github.com/H2Shami/Canopy-helicone-session), aby automatycznie rejestrować każdą konwersację Canopy jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja Canopy będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować Canopy tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować Canopy tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować Canopy tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem Canopy. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/Canopy).
:::
...
Aby korzystać z Ollama Cloud z Canopy:
...
3. Skopiuj klucz API do użycia w Canopy.
...
6. **Ważne**: Przed użyciem modeli chmurowych w Canopy musisz pobrać informacje o modelu lokalnie:
...
Canopy Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół Canopy. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>Canopy Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **Canopy Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API Canopy.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji Canopy.
...
5. Możesz je także dostosować za pomocą konfiguracji Canopy. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z Canopy:
...
Możesz także dostosować modele za pomocą konfiguracji Canopy. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z Canopy. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `canopy.json.guard` w katalogu projektu:
...
Pola `limit` pozwalają Canopy zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `canopy auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację Canopy i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji Canopy.
