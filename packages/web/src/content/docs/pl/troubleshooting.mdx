...
Aby debugować problemy z Canopy, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\Canopy\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `canopy --log-level DEBUG`.

---

## Przechowywanie danych

Canopy przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/Canopy/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\Canopy`
...
Canopy Desktop uruchamia w tle lokalny serwer Canopy (sidecar `Canopy-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `Canopy` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/Canopy/canopy.json.guardc` (lub `~/.config/Canopy/canopy.json.guard`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/Canopy/canopy.json.guardc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\Canopy\canopy.json.guardc`
...
Canopy może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/Canopy/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\Canopy\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby Canopy mógł ją odbudować.

1. Całkowicie zamknij Canopy Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/Canopy`
- **Linux**: usuń `~/.cache/Canopy` (lub uruchom `rm -rf ~/.cache/Canopy`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\Canopy`

3. Uruchom ponownie Canopy Desktop.

---

### Napraw problemy z połączeniem z serwerem

Canopy Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `canopy.json.guard(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.
...
W systemie Windows Canopy Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami Canopy.

---

### Brak powiadomień

Canopy Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla Canopy w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie Canopy Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji Canopy Desktop):

- `Canopy.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `Canopy.global.dat` i `Canopy.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
3. Upewnij się, że masz najnowszą wersję z `canopy upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `canopy models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\Canopy`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. Canopy dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\Canopy`

2. Uruchom ponownie kod Canopy, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to Canopy do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
Canopy wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
