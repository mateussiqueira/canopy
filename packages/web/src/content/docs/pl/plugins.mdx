---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć Canopy.
---

Wtyczki umożliwiające wyłączenie Canopy poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania Canopy.
...
- `.Canopy/plugins/` - Wtyczki na poziomie projektu
- `~/.config/Canopy/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/Canopy/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/Canopy/canopy.json.guard`)
2. Konfiguracja projektu (`canopy.json.guard`)
3. Globalny katalog wtyczek (`~/.config/Canopy/plugins/`)
4. Katalog wtyczek projektu (`.Canopy/plugins/`)
...
Canopy uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z Canopy do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do Canopy.
...
:::note
Jeśli korzystasz z aplikacji komputerowej Canopy, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj Canopy czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do Canopy:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod Canopy. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://canopy.dev/docs/sdk).
