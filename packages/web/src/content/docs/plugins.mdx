---
title: Plugins
description: Write your own plugins to extend Canopy.
---

Plugins allow you to extend Canopy by hooking into various events and customizing behavior. You can create plugins to add new features, integrate with external services, or modify Canopy's default behavior.
...
Canopy runs `bun install` at startup to install these. Your plugins and tools can then import them.
...
:::note
If you’re using the Canopy desktop app, it can send system notifications automatically when a response is ready or when a session errors.
:::
...
Levels: `debug`, `info`, `warn`, `error`. See [SDK documentation](https://canopy.dev/docs/sdk) for details.
