...
description: Skriv dine egne programtillegg for å utvide Canopy.
---

Plugins lar deg utvide Canopy ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til Canopy.
...
- `.Canopy/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/Canopy/node_modules/`.
...
1. Global konfigurasjon (`~/.config/Canopy/canopy.json.guard`)
2. Prosjektkonfigurasjon (`canopy.json.guard`)
3. Global plugin-katalog (`~/.config/Canopy/plugins/`)
4. Prosjektpluginkatalog (`.Canopy/plugins/`)
...
Canopy kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En Canopy SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide Canopy.
...
:::note
Hvis du bruker Canopy-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre Canopy fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til Canopy:
...
`tool`-hjelperen lager et tilpasset verktøy som Canopy kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for Canopy sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://canopy.dev/docs/sdk) for detaljer.
