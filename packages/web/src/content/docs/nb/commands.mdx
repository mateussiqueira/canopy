...
Opprett `.Canopy/commands/test.md`:
...
Du kan legge til egendefinerte kommandoer gjennom Canopy-konfigurasjonen eller ved å lage markdown-filer i katalogen `commands/`.

---

### JSON

Bruk alternativet `command` i Canopy [config](/docs/config):
...
Canopy inkluderer flere innebygde kommandoer som `/init`, `/undo`, `/redo`, `/share`, `/help`; [finn ut mer](/docs/tui#commands).
