---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i Canopy.
---

import config from "../../../../config.mjs"
export const console = config.console

Canopy bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i Canopy-konfigurasjonen.
...
i `~/.local/share/Canopy/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i Canopy
konfig.
...
Canopy Zen er en liste over modeller levert av Canopy-teamet som har vært
testet og verifisert for å fungere godt med Canopy. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med Canopy Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `Canopy Zen` og gå til [canopy.dev/auth](https://canopy.dev/zen).
...
Det fungerer som alle andre leverandører i Canopy og er helt valgfritt å bruke.

---

## Canopy Go

Canopy Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av Canopy-teamet som har vært
testet og verifisert for å fungere godt med Canopy.

1. Kjør kommandoen `/connect` i TUI, velg `Canopy Go`, og gå til [canopy.dev/auth](https://canopy.dev/zen).
...
Slik bruker du Amazon Bedrock med Canopy:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører Canopy:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `canopy.json.guard`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i Canopy støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere Canopy til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at Canopy skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom Canopy-konfigurasjonen.
...
for å lagre token i Canopy auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
Canopy bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse Canopy
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`canopy.json.guard` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [Canopy-gitlab-auth](https://www.npmjs.com/package/Canopy-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `canopy.json.guard`:
...
Slik bruker du GitHub Copilot-abonnementet med Canopy:
...
Slik bruker du Google Vertex AI med Canopy:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører Canopy.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom Canopy, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [Canopy-helicone-session](https://github.com/H2Shami/Canopy-helicone-session) plugin for automatisk å logge hver Canopy-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver Canopy-samtale oppført som en separat økt.
...
Du kan konfigurere Canopy for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere Canopy for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere Canopy for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for Canopy. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/Canopy) for detaljer.
:::
...
Slik bruker du Ollama Cloud med Canopy:
...
3. Kopier API-nøkkelen for bruk i Canopy.
...
6. **Viktig**: Før du bruker skymodeller i Canopy, må du hente modellinformasjonen lokalt:
...
Canopy Zen er en liste over testede og verifiserte modeller levert av Canopy-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>Canopy Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **Canopy Zen**.

   ```txt
   /connect
   ```

3. Skriv inn Canopy API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom Canopy-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom Canopy-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Canopy:
...
Du kan også tilpasse modeller gjennom Canopy-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med Canopy. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `canopy.json.guard`-filen i prosjektkatalogen:
...
`limit`-feltene lar Canopy forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `canopy auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk Canopy-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i Canopy-konfigurasjonen.
