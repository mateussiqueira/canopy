---
title: ACP-støtte
description: Bruk Canopy i en hvilken som helst ACP-kompatibel editor.
---

Canopy støtter [Agent Client Protocol](https://agentclientprotocol.com) eller (ACP), slik at du kan bruke den direkte i kompatible editorer og IDE-er.
...
For å bruke Canopy via ACP, konfigurer redigeringsprogrammet til å kjøre kommandoen `canopy acp`.

Kommandoen starter Canopy som en ACP-kompatibel underprosess som kommuniserer med editoren din over JSON-RPC via stdio.
...
For å åpne den, bruk den nye 'Canopy'-agenten i AI Chat-agentvelgeren.
...
For å bruke Canopy som en ACP-agent i [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), legg til følgende i Neovim-konfigurasjonen:
...
Denne konfigurasjonen setter opp CodeCompanion.nvim til å bruke Canopy som ACP-agent for chat.
...
Canopy fungerer på samme måte via ACP som i terminalen. Alle funksjoner støttes:
...
- MCP servere konfigurert i Canopy-konfigurasjonen
