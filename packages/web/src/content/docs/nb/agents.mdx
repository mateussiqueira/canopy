...
Det er to typer agenter i Canopy; primære agenter og underagenter.
...
Canopy kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
Canopy kommer med tre innebygde underagenter, **General**, **Explore** og **Scout**. Vi skal se på dette nedenfor.

---

## Innebygd

Canopy kommer med to innebygde primære agenter og tre innebygde underagenter.
...
En skrivebeskyttet agent for ekstern dokumentasjon og forskning på avhengigheter. Bruk denne når du trenger å klone et avhengighetsrepo inn i Canopy sin administrerte cache, inspisere kildekoden til et bibliotek eller kryssjekke lokal kode mot upstream-implementasjoner uten å endre arbeidsområdet ditt.
...
Konfigurer agenter i konfigurasjonsfilen `canopy.json.guard`:
...
- Globalt: `~/.config/Canopy/agents/`
- Per prosjekt: `.Canopy/agents/`

```markdown title="~/.config/Canopy/agents/review.md"
---
...
Hvis ingen temperatur er spesifisert, bruker Canopy modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale Canopy-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i Canopy-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [Canopy Zen](/docs/zen), vil du bruke `canopy/gpt-5.1-codex` for GPT 5.1 Codex.
...
```json title="canopy.json.guard" {3-6,9-12}
{
  "$schema": "https://canopy.dev/config.json",
  "tools": {
    "write": true,
...
```json title="canopy.json.guard" {3-5,8-10}
{
  "$schema": "https://canopy.dev/config.json",
  "permission": {
    "edit": "deny"
...
```json title="canopy.json.guard" {8}
{
  "$schema": "https://canopy.dev/config.json",
  "agent": {
    "build": {
...
```json title="canopy.json.guard" {6,7}
{
  "agent": {
    "deep-thinker": {
...
:::tip
Kjør `canopy models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/mateussiqueira/canopy).
:::
...
```markdown title="~/.config/Canopy/agents/docs-writer.md"
---
...
```markdown title="~/.config/Canopy/agents/security-auditor.md"
---
