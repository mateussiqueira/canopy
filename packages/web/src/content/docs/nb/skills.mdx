...
Agentferdigheter lar Canopy oppdage gjenbrukbare instruksjoner fra repo- eller hjemmekatalogen din.
...
Canopy søker etter disse stedene:

- Prosjektkonfigurasjon: `.Canopy/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/Canopy/skills/<name>/SKILL.md`
...
For prosjektlokale stier går Canopy opp fra din nåværende arbeidskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.Canopy/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/Canopy/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.Canopy/skills/git-release/SKILL.md` slik:
...
Canopy viser tilgjengelige ferdigheter i `skill`-verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved å bruke mønsterbaserte tillatelser i `canopy.json.guard`:
...
**For innebygde agenter** (i `canopy.json.guard`):
