...
Hvis serveren krever autentisering, vil Canopy be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `opencode mcp auth <server-name>`.
...
Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil Canopy lagre tokens sikkert i `~/.local/share/opencode/mcp-auth.json`.
