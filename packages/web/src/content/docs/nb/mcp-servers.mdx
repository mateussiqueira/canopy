...
Du kan legge til eksterne verktøy i Canopy ved å bruke _Model Context Protocol_ eller MCP. Canopy støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [Canopy Config](https://canopy.dev/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="canopy.json.guardc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/Canopy`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
```jsonc title="canopy.json.guardc" {15}
{
  "$schema": "https://canopy.dev/config.json",
  "mcp": {
    "my-local-mcp-server": {
...
Canopy håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil Canopy:
...
Hvis serveren krever autentisering, vil Canopy be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `canopy mcp auth <server-name>`.
...
```json title="canopy.json.guard" {7-11}
{
  "$schema": "https://canopy.dev/config.json",
  "mcp": {
    "my-oauth-server": {
...
```bash
Canopy mcp auth my-oauth-server
```
...
```bash
Canopy mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil Canopy lagre tokens sikkert i `~/.local/share/Canopy/mcp-auth.json`.
...
```json title="canopy.json.guard" {7}
{
  "$schema": "https://canopy.dev/config.json",
  "mcp": {
    "my-api-key-server": {
...
Canopy mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i Canopy, sammen med innebygde verktøy. Så du kan administrere dem gjennom Canopy-konfigurasjonen som alle andre verktøy.
...
```json title="canopy.json.guard" {14}
{
  "$schema": "https://canopy.dev/config.json",
  "mcp": {
    "my-mcp-foo": {
...
```json title="canopy.json.guard" {11, 14-18}
{
  "$schema": "https://canopy.dev/config.json",
  "mcp": {
    "my-mcp": {
...
```json title="canopy.json.guard" {4-8}
{
  "$schema": "https://canopy.dev/config.json",
  "mcp": {
    "sentry": {
...
```bash
Canopy mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble Canopy til Sentry-kontoen din.
...
```json title="canopy.json.guard" {4-7}
{
  "$schema": "https://canopy.dev/config.json",
  "mcp": {
    "context7": {
...
```json title="canopy.json.guard" {7-9}
{
  "$schema": "https://canopy.dev/config.json",
  "mcp": {
    "context7": {
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver Canopy-samtale oppført som en separat økt.
