---
title: CLI
description: Canopy CLI-alternativer og kommandoer.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Canopy CLI starter som standard [TUI](/docs/tui) når den kjøres uten noen argumenter.

```bash
Canopy
```

Men den godtar også kommandoer som dokumentert på denne siden. Dette lar deg samhandle med Canopy programmatisk.
...
Start Canopy-terminalbrukergrensesnittet.
...
Administrer agenter for Canopy.
...
Koble en terminal til en allerede kjørende Canopy-backend-server startet via `serve` eller `web` kommandoer.

```bash
Canopy attach [url]
```

Dette gjør det mulig å bruke TUI med en ekstern Canopy-backend. For eksempel:
...
| <nobr><code>{"--username"}</code></nobr> | `-u` | Brukernavn for grunnleggende autentisering (standard: `OPENCODE_SERVER_USERNAME` eller `Canopy`) |
...
Canopy drives av leverandørlisten på [Models.dev](https://models.dev), så du kan bruke `canopy auth login` til å konfigurere API-nøkler for enhver leverandør du vil bruke. Dette er lagret i `~/.local/share/Canopy/auth.json`.

```bash
Canopy auth login
```

Når Canopy starter opp, laster den leverandørene fra filen med påloggingsinformasjon. Og hvis det er noen nøkler definert i miljøene dine eller en `.env`-fil i prosjektet ditt.
...
Bruk `--refresh`-flagget for å oppdatere den bufrede modelllisten. Dette er nyttig når nye modeller er lagt til en leverandør og du vil se dem i Canopy.
...
Kjør Canopy i ikke-interaktiv modus ved å sende en melding direkte.
...
Du kan også koble til en kjørende `canopy serve`-forekomst for å unngå MCP-serveres kaldstarttid ved hver kjøring:
...
| <nobr><code>{"--attach"}</code></nobr>   |      | Koble til en kjørende Canopy-server (f.eks. http://localhost:4096)                               |
| <nobr><code>{"--password"}</code></nobr> | `-p` | Passord for grunnleggende autentisering (standard: `OPENCODE_SERVER_PASSWORD`)                     |
| <nobr><code>{"--username"}</code></nobr> | `-u` | Brukernavn for grunnleggende autentisering (standard: `OPENCODE_SERVER_USERNAME` eller `Canopy`) |
...
Start en headless Canopy-server for API-tilgang. Sjekk ut [server-dokumentene](/docs/server) for hele HTTP-grensesnittet.

```bash
Canopy serve
```

Dette starter en HTTP-server som gir API tilgang til Canopy-funksjonalitet uten TUI-grensesnittet. Sett `OPENCODE_SERVER_PASSWORD` for å aktivere HTTP grunnleggende autentisering (brukernavn er standard til `Canopy`).
...
Vis tokenbruk og kostnadsstatistikk for Canopy-øktene dine.
...
Importer øktdata fra en JSON-fil eller en Canopy share-URL.
...
Start en headless Canopy-server med et webgrensesnitt.

```bash
Canopy web
```

Dette starter en HTTP-server og åpner en nettleser for å få tilgang til Canopy via et nettgrensesnitt. Sett `OPENCODE_SERVER_PASSWORD` for å aktivere HTTP grunnleggende autentisering (brukernavn er standard til `Canopy`).
...
Avinstaller Canopy og fjern alle relaterte filer.
...
Oppdaterer Canopy til siste versjon eller en spesifikk versjon.
...
Canopy CLI bruker følgende globale flagg.
...
Canopy kan konfigureres ved hjelp av miljøvariabler.
...
| `OPENCODE_SERVER_USERNAME`            | streng | Overstyr grunnleggende autentiseringsbrukernavn (standard `Canopy`) |
