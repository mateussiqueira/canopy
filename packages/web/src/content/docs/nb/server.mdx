---
title: Tjener
description: Samhandle med Canopy-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `canopy serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en Canopy-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `Canopy.local` |
...
Sett `OPENCODE_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `Canopy` som standard, eller sett `OPENCODE_SERVER_USERNAME` for å overstyre det. Dette gjelder både `canopy serve` og `canopy web`.
...
Når du kjører `Canopy` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk Canopy-serveren til å samhandle med Canopy programmatisk.
:::

Denne arkitekturen lar Canopy støtte flere klienter og lar deg samhandle med Canopy programmatisk.

Du kan kjøre `canopy serve` for å starte en frittstående server. Hvis du har
Canopy TUI kjører, vil `canopy serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av Canopy [IDE](/docs/ide) plugins.
...
Canopy-serveren viser følgende APIer.
