---
title: Providers
description: Using any LLM provider in Canopy.
---

import config from "../../../config.mjs"
export const console = config.console

Canopy uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your Canopy config.
...
You can customize the providers through the `provider` section in your Canopy
config.
...
Canopy Zen is a list of models provided by the Canopy team that have been
tested and verified to work well with Canopy. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with Canopy Zen.
:::

1. Run the `/connect` command in the TUI, select `Canopy Zen`, and head to [canopy.dev/auth](https://canopy.dev/zen).
...
It works like any other provider in Canopy and is completely optional to use.

---

## Canopy Go

Canopy Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the Canopy team that have been
tested and verified to work well with Canopy.

1. Run the `/connect` command in the TUI, select `Canopy Go`, and head to [canopy.dev/auth](https://canopy.dev/zen).
...
To use Amazon Bedrock with Canopy:
...
Canopy. Anthropic explicitly prohibits this.

Previous versions of Canopy came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in Canopy with zero setup:
...
6. Run the `/models` command to select a model.
   ...
   You can also add models through your Canopy config.
...
Canopy supports two authentication methods:

- **OAuth (Recommended)** — Sign in to your DigitalOcean account; Canopy uses your DigitalOcean API token directly for inference and discovers your Inference Routers.
...
3. Your browser opens to authorize Canopy. Sign in and approve.

   :::note
   Canopy requests `genai:read` and `inference:query` OAuth scopes. Your DigitalOcean API token is used directly for inference — no separate Model Access Key is created.
   :::
...
Inference Routers let you define a routing policy across multiple models — picking the cheapest, fastest, or most appropriate model per request based on the task. After OAuth, Canopy surfaces each router as `router:<router-name>` in the model picker.

Selecting a router model is a drop-in replacement for any other model — Canopy forwards your request and DigitalOcean picks the underlying model based on your router's policy. Learn more about [Inference Routers](https://docs.digitalocean.com/products/inference/how-to/use-inference-router/)
...
GitLab Duo support in Canopy is experimental. Features, configuration, and
behavior may change in future releases.
:::

Canopy integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
Canopy uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock Canopy
...
When a `duo-workflow-*` model is selected, Canopy will:
...
4. Route tool execution requests through Canopy's permission-gated tool system
...
To use Google Vertex AI with Canopy:
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin to automatically log each Canopy conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each Canopy conversation listed as a separate session.
...
:::tip
Ollama can automatically configure itself for Canopy. See the [Ollama integration docs](https://docs.ollama.com/integrations/opencode) for details.
:::
...
To use Ollama Cloud with Canopy:
...
3. Copy the API key for use in Canopy.
...
6. **Important**: Before using cloud models in Canopy, you must pull the model information locally:
...
Canopy Zen is a list of tested and verified models provided by the Canopy team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>Canopy Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **Canopy Zen**.

   ```txt
   /connect
   ```

3. Enter your Canopy API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your Canopy config.
...
5. You can also customize them through your Canopy config. Here's an example of specifying a provider
...
5. Complete sign-in in the browser that opens. Canopy captures the OAuth callback automatically and stores the token — no copy/paste needed.
...
You can also customize models through your Canopy config. Here's an example of specifying provider routing order.
...
2. Select **xAI Grok OAuth (SuperGrok Subscription)**. Canopy opens xAI's consent screen in your browser and waits for the callback on `http://127.0.0.1:56121/callback`.
...
Canopy refreshes the OAuth access token automatically. Any Grok or X Premium plan that includes Grok API access works; you do not need a separate `XAI_API_KEY`.

#### Option B — SuperGrok device-code (headless / remote server / VPS)

Use this when Canopy is running somewhere a browser can't reach the loopback redirect: a VPS, a remote dev box over SSH, inside Docker, in CI, etc. No callback port is opened on the host running Canopy — instead xAI hands the CLI a short code that you type into a browser on any other device (laptop, phone, …).
...
2. Select **xAI Grok OAuth (Headless / Remote / VPS)**. Canopy prints a verification URL and a short user code.

   ```txt
   Open https://x.ai/device on any device and enter code: ABCD-1234
   ```

3. Open the URL on a device that has a browser (your laptop or phone), enter the code, and approve the consent screen. Canopy polls xAI's token endpoint and stores the resulting OAuth tokens once you approve. Token refresh works the same as Option A.
...
The `limit` fields allow Canopy to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `canopy auth list` to see if the credentials
...
2. For custom providers, check the Canopy config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your Canopy config.
