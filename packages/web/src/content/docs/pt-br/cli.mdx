---
title: CLI
description: Opções e comandos da CLI do Canopy.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

A CLI do Canopy, por padrão, inicia o [TUI](/docs/tui) quando executada sem argumentos.

```bash
Canopy
```

Mas também aceita comandos conforme documentado nesta página. Isso permite que você interaja com o Canopy programaticamente.
...
Inicie a interface de usuário do terminal do Canopy.
...
A CLI do Canopy também possui os seguintes comandos.

---

### agent

Gerencie agentes para o Canopy.
...
Anexe um terminal a um servidor backend do Canopy já em execução, iniciado via comandos `serve` ou `web`.

```bash
Canopy attach [url]
```

Isso permite usar o TUI com um backend Canopy remoto. Por exemplo:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuário de autenticação básica (padrão: `OPENCODE_SERVER_USERNAME` ou `Canopy`) |
...
O Canopy é alimentado pela lista de provedores em [Models.dev](https://models.dev), então você pode usar `canopy auth login` para configurar chaves de API para qualquer provedor que você gostaria de usar. Isso é armazenado em `~/.local/share/Canopy/auth.json`.

```bash
Canopy auth login
```

Quando o Canopy é iniciado, ele carrega os provedores do arquivo de credenciais. E se houver chaves definidas em seus ambientes ou em um arquivo `.env` em seu projeto.
...
Use a flag `--refresh` para atualizar a lista de modelos em cache. Isso é útil quando novos modelos foram adicionados a um provedor e você deseja vê-los no Canopy.
...
Execute o Canopy em modo não interativo passando um prompt diretamente.
...
```bash "Canopy run"
Canopy run Explique o uso de context em Go
```

Você também pode se anexar a uma instância em execução do `canopy serve` para evitar tempos de inicialização a frio do servidor MCP em cada execução:
...
Canopy run --attach http://localhost:4096 "Explique async/await em JavaScript"
```
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Anexe a um servidor Canopy em execução (por exemplo, http://localhost:4096)     |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Senha de autenticação básica (padrão: `OPENCODE_SERVER_PASSWORD`)                 |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuário de autenticação básica (padrão: `OPENCODE_SERVER_USERNAME` ou `Canopy`) |
...
Inicie um servidor Canopy sem cabeça para acesso à API. Confira a [documentação do servidor](/docs/server) para a interface HTTP completa.

```bash
Canopy serve
```

Isso inicia um servidor HTTP que fornece acesso à funcionalidade do Canopy sem a interface TUI. Defina `OPENCODE_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `Canopy`).
...
Liste todas as sessões do Canopy.
...
Mostre o uso de tokens e estatísticas de custo para suas sessões do Canopy.
...
Importe dados da sessão de um arquivo JSON ou URL de compartilhamento do Canopy.

```bash
Canopy import <file>
```

Você pode importar de um arquivo local ou de uma URL de compartilhamento do Canopy.
...
Inicie um servidor Canopy sem cabeça com uma interface web.

```bash
Canopy web
```

Isso inicia um servidor HTTP e abre um navegador para acessar o Canopy através de uma interface web. Defina `OPENCODE_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `Canopy`).
...
Desinstale o Canopy e remova todos os arquivos relacionados.
...
Atualiza o Canopy para a versão mais recente ou uma versão específica.
...
A CLI do Canopy aceita as seguintes flags globais.
...
O Canopy pode ser configurado usando variáveis de ambiente.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Substituir nome de usuário de autenticação básica (padrão `Canopy`) |
