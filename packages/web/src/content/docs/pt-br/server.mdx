...
O comando `canopy serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente opencode pode usar.
...
Defina `OPENCODE_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `opencode`, ou defina `OPENCODE_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `canopy serve` quanto ao `canopy web`.
...
Você pode executar `canopy serve` para iniciar um servidor autônomo. Se você tiver o TUI do opencode em execução, `canopy serve` iniciará um novo servidor.
