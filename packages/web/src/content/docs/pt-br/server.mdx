---
title: Servidor
description: Interaja com o servidor Canopy via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `canopy serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente Canopy pode usar.
...
```bash
Canopy serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `Canopy.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
Canopy serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `OPENCODE_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `Canopy`, ou defina `OPENCODE_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `canopy serve` quanto ao `canopy web`.

```bash
OPENCODE_SERVER_PASSWORD=your-password Canopy serve
```
...
Quando você executa `Canopy`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor Canopy para interagir com o Canopy programaticamente.
:::

Essa arquitetura permite que o Canopy suporte múltiplos clientes e permite que você interaja com o Canopy programaticamente.

Você pode executar `canopy serve` para iniciar um servidor autônomo. Se você tiver o TUI do Canopy em execução, `canopy serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do Canopy [IDE](/docs/ide).
...
O servidor Canopy expõe as seguintes APIs.
