---
title: Plugins
description: Escreva seus próprios plugins para estender o Canopy.
---

Plugins permitem que você estenda o Canopy conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do Canopy.
...
- `.Canopy/plugins/` - Plugins em nível de projeto
- `~/.config/Canopy/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/Canopy/node_modules/`.
...
1. Configuração global (`~/.config/Canopy/canopy.json.guard`)
2. Configuração do projeto (`canopy.json.guard`)
3. Diretório de plugins global (`~/.config/Canopy/plugins/`)
4. Diretório de plugins do projeto (`.Canopy/plugins/`)
...
O Canopy executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do Canopy para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o Canopy.
...
:::note
Se você estiver usando o aplicativo desktop Canopy, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o Canopy de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao Canopy:
...
O helper `tool` cria uma ferramenta personalizada que o Canopy pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o Canopy junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://canopy.dev/docs/sdk) para detalhes.
