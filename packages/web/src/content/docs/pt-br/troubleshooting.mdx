...
Para depurar problemas com o Canopy, comece verificando os logs e os dados locais que ele armazena no disco.
...
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.local\share\Canopy\log`
...
Você pode definir o nível de log com a opção de linha de comando `--log-level` para obter informações de depuração mais detalhadas. Por exemplo, `canopy --log-level DEBUG`.

---

## Armazenamento

O Canopy armazena dados de sessão e outros dados do aplicativo no disco em:

- **macOS/Linux**: `~/.local/share/Canopy/`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.local\share\Canopy`
...
O Canopy Desktop executa um servidor Canopy local (o sidecar `Canopy-cli`) em segundo plano. A maioria dos problemas é causada por um plugin com mau funcionamento, um cache corrompido ou uma configuração de servidor incorreta.
...
- Apenas macOS: menu `Canopy` -> **Recarregar Webview** (ajuda se a interface estiver em branco/congelada).
...
- **macOS/Linux**: `~/.config/Canopy/canopy.json.guardc` (ou `~/.config/Canopy/canopy.json.guard`)
- **macOS/Linux** (instalações mais antigas): `~/.local/share/Canopy/canopy.json.guardc`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\Canopy\canopy.json.guardc`
...
O Canopy também pode carregar plugins locais do disco. Mova-os temporariamente para fora do caminho (ou renomeie a pasta) e reinicie o aplicativo de desktop:

- **Plugins globais**
  - **macOS/Linux**: `~/.config/Canopy/plugins/`
  - **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\Canopy\plugins`
...
Se desativar plugins não ajudar (ou se a instalação de um plugin estiver travada), limpe o cache para que o Canopy possa reconstruí-lo.

1. Saia completamente do Canopy Desktop.
2. Exclua o diretório de cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> cole `~/.cache/Canopy`
- **Linux**: exclua `~/.cache/Canopy` (ou execute `rm -rf ~/.cache/Canopy`)
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.cache\Canopy`

3. Reinicie o Canopy Desktop.

---

### Corrigindo problemas de conexão com o servidor

O Canopy Desktop pode iniciar seu próprio servidor local (padrão) ou conectar-se a uma URL de servidor que você configurou.
...
Se seu `canopy.json.guard(c)` contiver uma seção `server`, remova-a temporariamente e reinicie o aplicativo de desktop.
...
No Windows, o Canopy Desktop requer o **WebView2 Runtime** do Microsoft Edge. Se o aplicativo abrir em uma janela em branco ou não iniciar, instale/atualize o WebView2 e tente novamente.

---

### Windows: Problemas gerais de desempenho

Se você estiver enfrentando desempenho lento, problemas de acesso a arquivos ou problemas no terminal no Windows, tente usar [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). O WSL fornece um ambiente Linux que funciona de forma mais integrada com os recursos do Canopy.

---

### Notificações não aparecendo

O Canopy Desktop só mostra notificações do sistema quando:

- as notificações estão habilitadas para o Canopy nas configurações do seu sistema operacional, e
...
1. Saia do Canopy Desktop.
2. Encontre e exclua estes arquivos (eles estão no diretório de dados do aplicativo Canopy Desktop):

- `Canopy.settings.dat` (URL do servidor padrão do desktop)
- `Canopy.global.dat` e `Canopy.workspace.*.dat` (estado da interface como servidores/projetos recentes)
...
Se você estiver enfrentando problemas com o Canopy:
...
3. Certifique-se de que você tem a versão mais recente com `canopy upgrade`
...
Para descobrir quais modelos você tem acesso, execute `canopy models`
...
2. Se o problema persistir, tente limpar sua configuração armazenada:
   ...
   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%\.local\share\Canopy`
...
Se você encontrar erros de chamada de API, isso pode ser devido a pacotes de provedores desatualizados. O Canopy instala dinamicamente pacotes de provedores (OpenAI, Anthropic, Google, etc.) conforme necessário e os armazena em cache localmente.
...
1. Limpe o cache do pacote de provedores:
   ...
   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%\.cache\Canopy`

2. Reinicie o Canopy para reinstalar os pacotes de provedores mais recentes

Isso forçará o Canopy a baixar as versões mais recentes dos pacotes de provedores, o que muitas vezes resolve problemas de compatibilidade com parâmetros de modelo e alterações na API.
...
O Canopy detectará se você está usando Wayland e preferirá `wl-clipboard`, caso contrário, tentará encontrar ferramentas de área de transferência na ordem: `xclip` e `xsel`.
