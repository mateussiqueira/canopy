---
title: Configuração
description: Usando a configuração JSON do Canopy.
---

Você pode configurar o Canopy usando um arquivo de configuração JSON.

---

## Formato

O Canopy suporta os formatos **JSON** e **JSONC** (JSON com Comentários).
...
1. **Configuração remota** (de `.well-known/Canopy`) - padrões organizacionais
2. **Configuração global** (`~/.config/Canopy/canopy.json.guard`) - preferências do usuário
3. **Configuração personalizada** (`OPENCODE_CONFIG` var de ambiente) - substituições personalizadas
4. **Configuração do projeto** (`canopy.json.guard` no projeto) - configurações específicas do projeto
5. **Diretórios `.Canopy`** - agentes, comandos, plugins
...
:::note
Os diretórios `.Canopy` e `~/.config/Canopy` usam **nomes no plural** para subdiretórios: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, e `themes/`. Nomes no singular (por exemplo, `agent/`) também são suportados para compatibilidade retroativa.
:::
...
As organizações podem fornecer configuração padrão através do endpoint `.well-known/Canopy`. Isso é buscado automaticamente quando você se autentica com um provedor que o suporta.
...
Coloque sua configuração global do Canopy em `~/.config/Canopy/canopy.json.guard`. Use a configuração global para preferências de todo o usuário, como provedores, modelos e permissões.

Para configurações específicas do TUI, use `~/.config/Canopy/tui.json`.
...
Adicione `canopy.json.guard` na raiz do seu projeto. A configuração do projeto tem a maior precedência entre os arquivos de configuração padrão - ela substitui tanto as configurações globais quanto as remotas.
...
Quando o Canopy é iniciado, ele procura um arquivo de configuração no diretório atual ou sobe até o diretório Git mais próximo.
...
Especifique um diretório de configuração personalizado usando a variável de ambiente `OPENCODE_CONFIG_DIR`. Este diretório será pesquisado por agentes, comandos, modos e plugins, assim como o diretório padrão `.Canopy`, e deve seguir a mesma estrutura.
...
O diretório personalizado é carregado após a configuração global e os diretórios `.Canopy`, então ele **pode substituir** suas configurações.

---

## Esquema

O esquema de configuração do servidor/tempo de execução é definido em [**`canopy.dev/config.json`**](https://canopy.dev/config.json).

A configuração do TUI usa [**`canopy.dev/tui.json`**](https://canopy.dev/tui.json).
...
Chaves legadas `theme`, `keybinds` e `tui` em `canopy.json.guard` estão obsoletas e são migradas automaticamente quando possível.
...
Você pode configurar as configurações do servidor para os comandos `canopy serve` e `canopy web` através da opção `server`.
...
- `mdns` - Habilitar descoberta de serviço mDNS. Isso permite que outros dispositivos na rede descubram seu servidor Canopy.
- `mdnsDomain` - Nome de domínio personalizado para o serviço mDNS. O padrão é `Canopy.local`. Útil para executar várias instâncias na mesma rede.
...
Você pode configurar os provedores e modelos que deseja usar em sua configuração do Canopy através das opções `provider`, `model` e `small_model`.
...
A opção `small_model` configura um modelo separado para tarefas leves, como geração de títulos. Por padrão, o Canopy tenta usar um modelo mais barato se um estiver disponível do seu provedor, caso contrário, ele recua para seu modelo principal.
...
Você também pode definir agentes usando arquivos markdown em `~/.config/Canopy/agents/` ou `.Canopy/agents/`. [Saiba mais aqui](/docs/agents).
...
O agente padrão deve ser um agente primário (não um subagente). Isso pode ser um agente embutido como `"build"` ou `"plan"`, ou um [agente personalizado](/docs/agents) que você definiu. Se o agente especificado não existir ou for um subagente, o Canopy recuará para `"build"` com um aviso.

Essa configuração se aplica a todas as interfaces: TUI, CLI (`canopy run`), aplicativo desktop e GitHub Action.
...
Você também pode definir comandos usando arquivos markdown em `~/.config/Canopy/commands/` ou `.Canopy/commands/`. [Saiba mais aqui](/docs/commands).
...
O Canopy fará o download automaticamente de quaisquer novas atualizações quando for iniciado. Você pode desabilitar isso com a opção `autoupdate`.
...
Por padrão, o Canopy **permite todas as operações** sem exigir aprovação explícita. Você pode mudar isso usando a opção `permission`.
...
[Plugins](/docs/plugins) estendem o Canopy com ferramentas, hooks e integrações personalizadas.

Coloque arquivos de plugin em `.Canopy/plugins/` ou `~/.config/Canopy/plugins/`. Você também pode carregar plugins do npm através da opção `plugin`.
...
Isso é útil quando você deseja restringir o Canopy para usar apenas provedores específicos, em vez de desabilitá-los um a um.
