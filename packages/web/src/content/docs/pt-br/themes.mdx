...
Com o Canopy, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o Canopy usa nosso próprio tema `Canopy`.
...
O Canopy vem com vários temas embutidos.
...
- Querem que o Canopy corresponda à aparência do seu terminal
...
O Canopy suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/Canopy/themes/*.json` ou `$XDG_CONFIG_HOME/Canopy/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.Canopy/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.Canopy/themes/*.json`
