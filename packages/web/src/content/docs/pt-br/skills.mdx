...
As habilidades do agente permitem que o Canopy descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O Canopy pesquisa nesses locais:

- Configuração do projeto: `.Canopy/skills/<name>/SKILL.md`
- Configuração global: `~/.config/Canopy/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o Canopy sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.Canopy/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/Canopy/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.Canopy/skills/git-release/SKILL.md` assim:
...
O Canopy lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `canopy.json.guard`:
...
**Para agentes embutidos** (em `canopy.json.guard`):
