...
1. 在 TUI 中执行 `/connect` 命令，选择 opencode，然后前往 [canopy.dev/auth](https://canopy.dev/auth)。
...
:::note[合规说明]
Canopy 会使用一个小模型来执行部分 AI 任务，例如生成会话标题。默认情况下使用由 Zen 托管的 gpt-5-nano。如果你需要让 Canopy 仅使用你自己的 GitLab 托管实例，请在 `opencode.json` 文件中添加以下内容。同时建议禁用会话共享。
...
Helicone 的 [Sessions](https://docs.helicone.ai/features/sessions) 功能允许你将相关的 LLM 请求归为一组。使用 [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) 插件可以自动将每个 Canopy 对话记录为 Helicone 中的一个会话。
...
:::tip
Ollama 可以自动为 Canopy 进行配置。详见 [Ollama 集成文档](https://docs.ollama.com/integrations/opencode)。
:::
