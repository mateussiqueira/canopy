...
Создайте `.Canopy/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию Canopy или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем Canopy [config](/docs/config):
...
- Глобальный: `~/.config/Canopy/commands/`
- Для каждого проекта: `.Canopy/commands/`
...
Canopy включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
