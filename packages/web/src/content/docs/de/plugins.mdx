---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um Canopy zu erweitern.
---

Mit Plugins können Sie Canopy erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von Canopy zu ändern.
...
- `.Canopy/plugins/` – Plugins auf Projektebene
- `~/.config/Canopy/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/Canopy/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/Canopy/canopy.json.guard`)
2. Projektkonfiguration (`canopy.json.guard`)
3. Globales Plugin-Verzeichnis (`~/.config/Canopy/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.Canopy/plugins/`)
...
Canopy führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein Canopy-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie Canopy erweitern können.
...
:::note
Wenn Sie die Canopy-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass Canopy `.env`-Dateien liest:
...
Plugins können Canopy auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das Canopy aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für Canopy zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://canopy.dev/docs/sdk).
