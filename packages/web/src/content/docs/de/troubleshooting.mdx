...
Wenn Canopy Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\Canopy\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `canopy --log-level DEBUG`.

---

## Speicher

Canopy speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/Canopy/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\Canopy`
...
Canopy Desktop startet im Hintergrund einen lokalen Canopy-Server (`Canopy-cli`-Sidecar).
...
- Nur macOS: `Canopy`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/Canopy/canopy.json.guardc` (oder `~/.config/Canopy/canopy.json.guard`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/Canopy/canopy.json.guardc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\Canopy\canopy.json.guardc`
...
Canopy kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/Canopy/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\Canopy\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit Canopy ihn neu aufbauen kann.

1. Beende Canopy Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/Canopy`
- **Linux**: loesche `~/.cache/Canopy` (oder fuehre aus: `rm -rf ~/.cache/Canopy`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\Canopy`

3. Starte Canopy Desktop neu.

---

### Server-Verbindungsprobleme beheben

Canopy Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `canopy.json.guard(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.
...
Unter Windows benoetigt Canopy Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit Canopy-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

Canopy Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer Canopy in den OS-Einstellungen aktiviert sind, und
...
1. Beende Canopy Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von Canopy Desktop):

- `Canopy.settings.dat` (desktop default server URL)
- `Canopy.global.dat` und `Canopy.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit Canopy hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `canopy models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\Canopy`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. Canopy installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\Canopy`

2. Starte Canopy neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt Canopy, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
Canopy erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
