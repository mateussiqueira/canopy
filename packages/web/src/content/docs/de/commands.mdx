...
Erstellen Sie `.Canopy/commands/test.md`:

```md title=".Canopy/commands/test.md"
---
...
Sie können benutzerdefinierte Befehle über die Canopy-Konfiguration hinzufügen oder indem Sie Markdown-Dateien im Verzeichnis `commands/` erstellen.

---

### JSON

Verwenden Sie die Option `command` in Ihrem Canopy [config](/docs/config):

```json title="canopy.json.guardc" {4-12}
{
...
- Global: `~/.config/Canopy/commands/`
- Pro Projekt: `.Canopy/commands/`

```markdown title="~/.config/Canopy/commands/test.md"
---
description: Run tests with coverage
agent: build
model: anthropic/claude-3-5-sonnet-20241022
...
```md title=".Canopy/commands/component.md"
---
...
```md title=".Canopy/commands/create-file.md"
---
...
```md title=".Canopy/commands/analyze-coverage.md"
---
...
```md title=".Canopy/commands/review-changes.md"
---
...
```md title=".Canopy/commands/review-component.md"
---
...
Canopy enthält mehrere integrierte Befehle wie `/init`, `/undo`, `/redo`, `/share`, `/help`; [mehr erfahren](/docs/tui#commands).
