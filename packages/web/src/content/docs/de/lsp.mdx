---
title: LSP Server
description: Canopy lässt sich in Ihren LSP-Server integrieren.
---

Canopy kann sich in Language Server Protocol (LSP)-Server integrieren, um Diagnosen als Feedback für den Agenten zu verwenden.

---

## Integriert

Canopy verfügt über mehrere integrierte LSP-Server für gängige Sprachen:
...
Wenn LSP aktiviert ist und Canopy eine Datei öffnet, geschieht Folgendes:
...
Sie können LSP-Server über den Abschnitt `lsp` in Ihrer Canopy-Konfiguration aktivieren und anpassen.
...
```json title="canopy.json.guard" {5-7}
{
  "$schema": "https://canopy.dev/config.json",
  "lsp": {
    "rust": {
...
```json title="canopy.json.guard" {5-9}
{
  "$schema": "https://canopy.dev/config.json",
  "lsp": {
    "typescript": {
