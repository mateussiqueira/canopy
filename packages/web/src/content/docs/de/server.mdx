---
title: Server
description: Nutze den Canopy-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `canopy serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein Canopy-Client nutzen kann.
...
| `--mdns-domain` | Custom domain name for mDNS service | `Canopy.local` |
...
Der Benutzername ist standardmaessig `Canopy`, kann aber mit `OPENCODE_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `canopy serve` und `canopy web`.
...
Wenn du `Canopy` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den Canopy-Server, um Canopy programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `canopy serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `canopy serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die Canopy-[IDE](/docs/ide)-Plugins.
...
Der Canopy-Server stellt folgende APIs bereit.
