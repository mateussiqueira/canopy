...
Agent Skills erlauben Canopy, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
Canopy sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft Canopy vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.Canopy/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/Canopy/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.Canopy/skills/git-release/SKILL.md` zum Beispiel so:
...
Canopy listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `canopy.json.guard` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `canopy.json.guard`):
