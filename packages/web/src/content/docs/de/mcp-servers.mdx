...
Sie können externe Tools zu Canopy hinzufügen, indem Sie das _Model Context Protocol_ oder MCP verwenden. Canopy unterstützt sowohl lokale als auch Remote-Server.
...
Sie können den MCP-Server in Ihrem [Canopy Config](https://canopy.dev/docs/config/) unter `mcp` definieren. Fügen Sie jedem MCP einen eindeutigen Namen hinzu. Sie können bei der Eingabe von LLM namentlich auf diesen MCP verweisen.
...
Organisationen können über ihren `.well-known/Canopy`-Endpunkt Standard-MCP-Server bereitstellen. Diese Server sind möglicherweise standardmäßig deaktiviert, sodass Benutzer sich für den Server entscheiden können, die sie benötigen.
...
Canopy übernimmt automatisch die OAuth-Authentifizierung für Remote-MCP-Server. Wenn ein Server eine Authentifizierung erfordert, wird Canopy Folgendes tun:
...
Wenn der Server eine Authentifizierung erfordert, werden Sie von Canopy beim ersten Versuch, ihn zu verwenden, zur Authentifizierung autorisiert. Wenn nicht, können Sie [manually trigger the flow](#authenticating) mit `canopy mcp auth <server-name>` verwenden.
...
Der Befehl `mcp auth` öffnet Ihren Browser zur Autorisierung. Nach der Autorisierung speichert Canopy die Token sicher in `~/.local/share/Canopy/mcp-auth.json`.
...
Ihre MCPs sind neben integrierten Tools auch als Tools in Canopy verfügbar. Sie können sie also wie jedes andere Tool über die Canopy-Konfiguration verwalten.
...
Dadurch wird ein Browserfenster geöffnet, um den OAuth-Ablauf abzuschließen und Canopy mit Ihrem Sentry-Konto zu verbinden.
