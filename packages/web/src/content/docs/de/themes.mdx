...
Mit Canopy kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet Canopy das `Canopy`-Theme.
...
Canopy liefert mehrere eingebaute Themes mit.
...
- Canopy optisch ans Terminal anpassen wollen
...
Canopy unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/Canopy/themes/*.json` oder `$XDG_CONFIG_HOME/Canopy/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.Canopy/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.Canopy/themes/*.json`
