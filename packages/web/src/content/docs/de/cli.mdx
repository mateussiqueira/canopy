---
title: CLI
description: Canopy CLI Optionen und Befehle.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Der Canopy CLI startet standardmäßig das [TUI](/docs/tui), wenn er ohne Argumente ausgeführt wird.

```bash
Canopy
```

Es akzeptiert aber auch Befehle, wie auf dieser Seite dokumentiert. Dadurch können Sie programmgesteuert mit Canopy interagieren.
...
Starten Sie die Terminalbenutzeroberfläche Canopy.
...
Der Canopy CLI verfügt außerdem über die folgenden Befehle.

---

### agent

Agenten für Canopy verwalten.
...
Schließen Sie ein Terminal an einen bereits laufenden Canopy-Backend-Server an, der über die Befehle `serve` oder `web` gestartet wurde.

```bash
Canopy attach [url]
```

Dies ermöglicht die Verwendung von TUI mit einem Remote-Backend Canopy. Zum Beispiel:
...
| <nobr><code>{"--username"}</code></nobr> | `-u` | Basic-Auth-Benutzername (standardmäßig `OPENCODE_SERVER_USERNAME` oder `Canopy`) |
...
Canopy wird von der Anbieterliste bei [Models.dev](https://models.dev) unterstützt, sodass Sie `canopy auth login` verwenden können, um API-Schlüssel für jeden Anbieter zu konfigurieren, den Sie verwenden möchten. Dies wird in `~/.local/share/Canopy/auth.json` gespeichert.

```bash
Canopy auth login
```

Wenn Canopy startet, lädt es den Anbieter aus der Anmeldeinformationsdatei. Und ob in Ihren Umgebungen Schlüssel sind oder in Ihrem Projekt eine `.env`-Datei vorhanden ist.
...
Verwenden Sie das Flag `--refresh`, um die zwischengespeicherte Modellliste zu aktualisieren. Dies ist nützlich, wenn einem Anbieter neue Modelle hinzugefügt wurden und Sie diese in Canopy sehen möchten.
...
Führen Sie Canopy im nicht interaktiven Modus aus, indem Sie direkt eine Eingabeaufforderung übergeben.
...
Sie können auch eine Verbindung zu einer laufenden `canopy serve`-Instanz herstellen, um bei jedem Lauf Kaltstartzeiten für den MCP-Server zu vermeiden:
...
| <nobr><code>{"--attach"}</code></nobr>   |      | An einen laufenden Canopy-Server anschließen (z.B. http://localhost:4096)                         |
| <nobr><code>{"--password"}</code></nobr> | `-p` | Basic-Auth-Passwort (standardmäßig `OPENCODE_SERVER_PASSWORD`)                                      |
| <nobr><code>{"--username"}</code></nobr> | `-u` | Basic-Auth-Benutzername (standardmäßig `OPENCODE_SERVER_USERNAME` oder `Canopy`)                  |
...
Starten Sie einen Headless-Canopy-Server für den API-Zugriff. Sehen Sie sich die [Server-Dokumentation](/docs/server) für die vollständige HTTP-Schnittstelle an.

```bash
Canopy serve
```

Dadurch wird ein HTTP-Server gestartet, der API-Zugriff auf Canopy-Funktionalität ohne die TUI-Schnittstelle bietet. Legen Sie `OPENCODE_SERVER_PASSWORD` fest, um die HTTP-Basisauthentifizierung zu aktivieren (Benutzername ist standardmäßig `Canopy`).
...
Listen Sie alle Canopy-Sitzungen auf.
...
Zeigen Sie Token-Nutzungs- und Kostenstatistiken für Ihre Canopy-Sitzungen an.
...
Importieren Sie Sitzungsdaten aus einer JSON-Datei oder einer Canopy-Freigabe-URL.

```bash
Canopy import <file>
```

Sie können aus einer lokalen Datei oder einer Canopy-Freigabe-URL importieren.
...
Starten Sie einen Headless-Canopy-Server mit einer Weboberfläche.

```bash
Canopy web
```

Dadurch wird ein HTTP-Server gestartet und ein Webbrowser geöffnet, um über eine Webschnittstelle auf Canopy zuzugreifen. Legen Sie `OPENCODE_SERVER_PASSWORD` fest, um die HTTP-Basisauthentifizierung zu aktivieren (Benutzername ist standardmäßig `Canopy`).
...
Deinstallieren Sie Canopy und entfernen Sie alle zugehörigen Dateien.
...
Aktualisiert Canopy auf die neueste Version oder eine bestimmte Version.
...
Der Canopy CLI akzeptiert die folgenden globalen Flags.
...
Canopy kann mithilfe von Umgebungsvariablen konfiguriert werden.
...
| `OPENCODE_SERVER_USERNAME`            | Zeichenfolge    | Benutzernamen für die Basisauthentifizierung überschreiben (Standard `Canopy`) |
