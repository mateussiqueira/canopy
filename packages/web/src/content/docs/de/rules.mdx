---
title: Regeln
description: Lege eigene Anweisungen fuer Canopy fest.
---

Du kannst Canopy ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in Canopy den Befehl `/init` aus.
...
So kann Canopy sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht Canopy sie zu erweitern.
...
Canopy kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/Canopy/AGENTS.md` speichern.
Diese gelten in allen Canopy-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt Canopy dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/Canopy/AGENTS.md` existiert)
...
Beim Start sucht Canopy in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/Canopy/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `canopy.json.guard` oder in `~/.config/Canopy/canopy.json.guard` angeben.
...
Canopy parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit canopy.json.guard

Empfohlen ist das `instructions`-Feld in der `canopy.json.guard`:
...
Du kannst Canopy explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `canopy.json.guard` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
