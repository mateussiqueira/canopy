---
title: Eklentiler
description: Canopy'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek Canopy'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya Canopy'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.Canopy/plugins/` - Proje düzeyinde eklentiler
- `~/.config/Canopy/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/Canopy/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/Canopy/canopy.json.guard`)
2. Project config (`canopy.json.guard`)
3. Global eklenti dizini (`~/.config/Canopy/plugins/`)
4. Proje eklenti dizini (`.Canopy/plugins/`)
...
Canopy bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir Canopy SDK istemcisi.
...
Canopy'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
Canopy masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
Canopy'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca Canopy'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, Canopy'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra Canopy'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://canopy.dev/docs/sdk) bakın.
