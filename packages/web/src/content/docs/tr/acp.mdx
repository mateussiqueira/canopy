---
title: ACP Desteği
description: Canopy'u ACP uyumlu herhangi bir editörde kullanın.
---

Canopy, [Agent Client Protocol](https://agentclientprotocol.com) veya (ACP) dosyalarını destekleyerek onu doğrudan uyumlu editörlerde ve IDE'lerde kullanmanızı sağlar.
...
Canopy'u ACP aracılığıyla kullanmak için düzenleyicinizi `canopy acp` komutunu çalıştıracak şekilde yapılandırın.

Komut, Canopy'u, editörünüzle stdio aracılığıyla JSON-RPC üzerinden iletişim kuran ACP uyumlu bir alt süreç olarak başlatır.
...
Açmak için AI Chat agent seçicisindeki yeni 'Canopy' ajanını kullanın.
...
Canopy'u [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim)'de ACP ajanı olarak kullanmak için Neovim yapılandırmanıza aşağıdakileri ekleyin:
...
Bu yapılandırma, CodeCompanion'ı Canopy'u sohbet için ACP ajanı olarak kullanacak şekilde ayarlar.
...
Canopy, terminalde olduğu gibi ACP aracılığıyla aynı şekilde çalışır. Tüm özellikler desteklenir:
...
- Canopy yapılandırmanızda yapılandırılmış MCP sunucuları
