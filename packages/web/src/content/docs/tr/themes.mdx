...
Canopy ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak Canopy kendi `Canopy` temasını kullanır.
...
Canopy birden fazla yerleşik temayla gelir.
...
- Canopy'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
Canopy, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/Canopy/themes/*.json` veya `$XDG_CONFIG_HOME/Canopy/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.Canopy/themes/*.json`
4. **Geçerli çalışma dizini** - `./.Canopy/themes/*.json`
