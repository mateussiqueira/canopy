...
Canopy'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
Canopy, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
Canopy, **General**, **Explore** ve **Scout** olmak üzere üç yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

Canopy iki yerleşik birincil agent ve üç yerleşik alt agent ile birlikte gelir.
...
Agent'ları `canopy.json.guard` yapılandırma dosyanızda yapılandırın:
...
Sıcaklık belirtilmezse Canopy modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global Canopy yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
Canopy yapılandırmanızdaki model kimliği `provider/model-id` biçimini kullanır. Örneğin, [Canopy Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `canopy/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `canopy models` komutunu çalıştırın.
:::
...
:::tip
Paylaşmak istediğiniz bir agent'ınız var mı? [Submit a PR](https://github.com/mateussiqueira/canopy).
:::
