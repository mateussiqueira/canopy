...
Canopy ile ilgili bir sorunu ayiklamak icin once loglari ve diskte tuttugu yerel verileri kontrol edin.
...
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\Canopy\log` yapistirin

Log dosyalari zaman damgasiyla adlandirilir (ornegin `2025-01-09T123456.log`) ve en yeni 10 log dosyasi tutulur.

Daha ayrintili ayiklama bilgisi icin `--log-level` komut satiri secenegini kullanabilirsiniz. Ornek: `canopy --log-level DEBUG`.

---

## Depolama

Canopy, oturum verilerini ve diger uygulama verilerini diskte su konumda saklar:

- **macOS/Linux**: `~/.local/share/Canopy/`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\Canopy` yapistirin
...
Canopy Desktop arka planda yerel bir Canopy sunucusu (`Canopy-cli` sidecar) calistirir. Sorunlarin cogu bozuk bir plugin, hasarli cache veya hatali sunucu ayarindan kaynaklanir.

### Hızlı kontroller

- Uygulamayi tamamen kapatip yeniden acin
- Uygulama hata ekrani gosteriyorsa **Restart**'a tiklayip hata detaylarini kopyalayin
- Yalnizca macOS: `Canopy` menusu -> **Reload Webview** (arayuz bos/donukse yardimci olur)
...
- **macOS/Linux**: `~/.config/Canopy/canopy.json.guardc` (veya `~/.config/Canopy/canopy.json.guard`)
- **macOS/Linux** (eski kurulumlar): `~/.local/share/Canopy/canopy.json.guardc`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\Canopy\canopy.json.guardc` yapistirin
...
Canopy diskten yerel plugin de yukleyebilir. Bu dizinleri gecici olarak tasiyin (veya klasoru yeniden adlandirin) ve uygulamayi yeniden baslatin:

- **Global plugins**
  - **macOS/Linux**: `~/.config/Canopy/plugins/`
  - **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\Canopy\plugins` yapistirin
...
Pluginleri kapatmak ise yaramazsa (veya plugin kurulumu takili kaldiysa), Canopy'un cache'i yeniden olusturmasi icin cache'i temizleyin.

1. Canopy Desktop'u tamamen kapatin
2. Cache dizinini silin:

- **macOS**: Finder -> `Cmd+Shift+G` -> `~/.cache/Canopy`
- **Linux**: `~/.cache/Canopy` dizinini silin (veya `rm -rf ~/.cache/Canopy` calistirin)
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.cache\Canopy` yapistirin

3. Canopy Desktop'u yeniden baslatin

---

### Sunucu bağlantı sorunlarını düzeltin

Canopy Desktop ya kendi yerel sunucusunu baslatir (varsayilan) ya da sizin tanimladiginiz bir sunucu URL'sine baglanir.
...
`canopy.json.guard(c)` dosyanizda `server` bolumu varsa gecici olarak kaldirin ve desktop uygulamasini yeniden baslatin.
...
Windows'ta Canopy Desktop, Microsoft Edge **WebView2 Runtime** gerektirir. Uygulama bos pencereyle aciliyorsa veya hic baslamiyorsa WebView2'yi kurup/guncelleyip tekrar deneyin.

---

### Windows: Genel performans sorunları

Windows'ta yavaslik, dosya erisim sorunlari veya terminal problemleri yasiyorsaniz [WSL (Windows Subsystem for Linux)](/docs/windows-wsl) kullanmayi deneyin. WSL, Canopy ozellikleriyle daha sorunsuz calisan bir Linux ortami saglar.

---

### Bildirimler görünmüyor

Canopy Desktop sistem bildirimlerini yalnizca su durumlarda gosterir:

- Isletim sistemi ayarlarinizda Canopy icin bildirimler etkinse
...
1. Canopy Desktop'u kapatin
2. Su dosyalari bulun ve silin (Canopy Desktop uygulama veri dizininde yer alirlar):

- `Canopy.settings.dat` (desktop varsayilan sunucu URL'si)
- `Canopy.global.dat` ve `Canopy.workspace.*.dat` (son sunucular/projeler gibi UI durumu)
...
Canopy ile ilgili bir sorun yasiyorsaniz:
...
3. `canopy upgrade` ile en guncel surumu kullandiginizdan emin olun
...
Erisiminiz olan modelleri gormek icin `canopy models` calistirin.
...
2. Sorun surerse kayitli konfigurasyonu temizlemeyi deneyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.local\share\Canopy`
...
API cagrisi hatalari aliyorsaniz bunun nedeni guncel olmayan provider paketleri olabilir. Canopy, provider paketlerini (OpenAI, Anthropic, Google vb.) gerektikce dinamik yukler ve yerelde onbellekler.
...
1. Provider paket cache'ini temizleyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.cache\Canopy`

2. En guncel provider paketlerini yeniden kurmak icin Canopy'u yeniden baslatin

Bu, Canopy'un en yeni provider paket surumlerini indirmesini zorlar ve model parametresi/API degisikliklerinden kaynakli uyumsuzluklari cogu zaman cozer.
...
Canopy, Wayland kullandiginizi algilarsa `wl-clipboard` tercihi yapar. Aksi halde sirayla `xclip` ve `xsel` araclarini arar.
