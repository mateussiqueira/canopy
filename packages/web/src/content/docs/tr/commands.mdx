...
`.Canopy/commands/test.md` oluştur:
...
Canopy işlemleri aracılığıyla veya `commands/` dizininde işaretleme dosyalarının oluşturulması özel komutlar ile yapılabilir.

---

### JSON

Canopy [config](/docs/config)'deki `command` seçeneğini kullanın:
...
- Küresel: `~/.config/Canopy/commands/`
- Proje başına: `.Canopy/commands/`
...
Canopy, `/init`, `/undo`, `/redo`, `/share`, `/help` gibi çeşitli yerleşik komutlar içerir; [learn more](/docs/tui#commands).
