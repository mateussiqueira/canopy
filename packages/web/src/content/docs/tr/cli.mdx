---
title: CLI
description: Canopy CLI seçenekleri ve komutları.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Canopy CLI herhangi bir argüman olmadan çalıştırıldığında varsayılan olarak [TUI](/docs/tui)'yi başlatır.

```bash
Canopy
```

Ancak bu sayfada belgelendiği gibi komutları da kabul eder. Bu, Canopy ile programlı olarak etkileşim kurmanıza olanak tanır.
...
Canopy terminal kullanıcı arayüzünü başlatır.
...
Canopy CLI ayrıca aşağıdaki komutlara da sahiptir.

---

### agent

Canopy için agent'ları yönetin.
...
`serve` veya `web` komutlarıyla başlatılan, halihazırda çalışan bir Canopy arka uç sunucusuna bir terminal ekleyin.

```bash
Canopy attach [url]
```

Bu, TUI öğesinin uzak bir Canopy arka ucuyla kullanılmasına olanak tanır. Örneğin:
...
| <nobr><code>{"--username"}</code></nobr> | `-u` | Temel kimlik doğrulama kullanıcı adı (varsayılan: `OPENCODE_SERVER_USERNAME` veya `Canopy`) |
...
Canopy, [Models.dev](https://models.dev) adresindeki sağlayıcı listesi tarafından desteklenmektedir, dolayısıyla kullanmak istediğiniz herhangi bir sağlayıcı için API anahtarlarını ayarlamak üzere `canopy auth login` kullanabilirsiniz. Bu `~/.local/share/Canopy/auth.json`'de saklanıyor.

```bash
Canopy auth login
```

Canopy başlatıldığında sağlayıcıları kimlik bilgileri dosyasından yükler. Ayrıca ortam değişkenlerindeki veya projenizdeki `.env` dosyasındaki anahtarları da kullanır.
...
Önbelleğe alınan model listesini güncellemek için `--refresh` bayrağını kullanın. Bu, bir sağlayıcıya yeni modeller eklendiğinde ve bunları Canopy'da görmek istediğinizde kullanışlıdır.
...
Doğrudan bir istem ileterek Canopy'u etkileşimli olmayan modda çalıştırın.
...
Ayrıca, her çalıştırmada MCP sunucusunun soğuk önyükleme sürelerini önlemek için çalışan bir `canopy serve` örneğine de ekleyebilirsiniz:
...
| <nobr><code>{"--attach"}</code></nobr>   |      | Çalışan bir Canopy sunucusuna ekleyin (ör. http://localhost:4096)                           |
| <nobr><code>{"--password"}</code></nobr> | `-p` | Temel kimlik doğrulama parolası (varsayılan: `OPENCODE_SERVER_PASSWORD`)                      |
| <nobr><code>{"--username"}</code></nobr> | `-u` | Temel kimlik doğrulama kullanıcı adı (varsayılan: `OPENCODE_SERVER_USERNAME` veya `Canopy`) |
...
API erişimi için headless bir Canopy sunucusu başlatır. Tam HTTP arayüzü için [server docs](/docs/server) sayfasına bakın.

```bash
Canopy serve
```

Bu, TUI arayüzü olmadan Canopy işlevselliğine API erişimi sağlayan bir HTTP sunucusunu başlatır. HTTP temel kimlik doğrulamasını etkinleştirmek için `OPENCODE_SERVER_PASSWORD` öğesini ayarlayın (kullanıcı adı varsayılan olarak `Canopy` şeklindedir).
...
Tüm Canopy oturumlarını listeleyin.
...
Canopy oturumlarınız için belirteç kullanımı ve maliyet istatistiklerini gösterin.
...
Bir JSON dosyasından veya Canopy paylaşımından URL oturum verilerini içe aktarın.

```bash
Canopy import <file>
```

Yerel bir dosyadan veya Canopy paylaşımından (URL) içe aktarabilirsiniz.
...
Web arayüzüyle başsız bir Canopy sunucusu başlatın.

```bash
Canopy web
```

Bu, bir HTTP sunucusunu başlatır ve bir web arayüzü aracılığıyla Canopy'a erişmek için bir web tarayıcısı açar. HTTP temel kimlik doğrulamasını etkinleştirmek için `OPENCODE_SERVER_PASSWORD` öğesini ayarlayın (kullanıcı adı varsayılan olarak `Canopy` şeklindedir).
...
Canopy'u kaldırın ve ilgili tüm dosyaları kaldırın.
...
Canopy'u en son sürüme veya belirli bir sürüme günceller.
...
Canopy CLI aşağıdaki global bayrakları destekler.
...
Canopy ortam değişkenleri kullanılarak yapılandırılabilir.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Temel kimlik doğrulama kullanıcı adını geçersiz kıl (varsayılan `Canopy`) |
