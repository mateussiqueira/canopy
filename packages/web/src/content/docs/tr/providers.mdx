---
title: Sağlayıcılar
description: Canopy'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

Canopy, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı Canopy yapılandırmanızda yapılandırın.
...
`~/.local/share/Canopy/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları Canopy'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
Canopy Zen, Canopy ekibi tarafından sağlanan modellerin bir listesidir.
Canopy ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz Canopy Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, Canopy'u seçin ve [canopy.dev/auth](https://canopy.dev/auth)'ye gidin.
...
Canopy'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## Canopy Go

Canopy Go, Canopy ile iyi çalıştığı test edilmiş ve doğrulanmış, Canopy ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `Canopy Go`'yu seçin ve [canopy.dev/auth](https://canopy.dev/zen) adresine gidin.
...
Amazon Bedrock'u Canopy ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   Canopy'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `canopy.json.guard` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi Canopy'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
Canopy'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://atomic.chat) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   Canopy'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
to store token in Canopy auth storage.
:::

##### Kendi Kendine Barındırılan GitLab

:::note[uyumluluk notu]
Canopy, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. Canopy'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`canopy.json.guard` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
Daha fazla belge [Canopy-gitlab-auth](https://www.npmjs.com/package/Canopy-gitlab-auth) ana sayfasında.

##### Yapılandırma

`canopy.json.guard` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi Canopy'la kullanmak için:
...
Google Vertex AI'yi Canopy ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları Canopy çalıştırılırken ayarlayın.
...
Helicone'un Canopy aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her Canopy görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [Canopy-helicone-session](https://github.com/H2Shami/Canopy-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her Canopy görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
Canopy'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla Canopy'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla Canopy'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini Canopy için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/Canopy)'a bakın.
:::
...
Ollama Cloud'u Canopy ile kullanmak için:
...
3. Canopy'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: Canopy'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
Canopy Zen, Canopy ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>Canopy Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **Canopy Zen**'i arayın.

   ```txt
   /connect
   ```

3. Canopy API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   Canopy yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca Canopy yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
Modelleri Canopy yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı Canopy ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `canopy.json.guard` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları Canopy'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `canopy auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için Canopy yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, Canopy yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
