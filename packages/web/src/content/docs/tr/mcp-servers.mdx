...
_Model Bağlam Protokolü_ veya MCP kullanarak Canopy'a harici araçlar ekleyebilirsiniz. Canopy hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [Canopy Config](https://canopy.dev/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/Canopy` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
Canopy, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde Canopy şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa Canopy, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `canopy mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından Canopy, belirteçleri `~/.local/share/Canopy/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra Canopy'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi Canopy yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve Canopy'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
