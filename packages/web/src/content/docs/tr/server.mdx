...
`canopy serve` komutu, opencode istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `OPENCODE_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `opencode` değeridir; değiştirmek isterseniz `OPENCODE_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `canopy serve` hem de `canopy web` için geçerlidir.
...
`canopy serve` ile bağımsız bir sunucu başlatabilirsiniz. opencode TUI açıksa, `canopy serve` yeni bir sunucu başlatır.
