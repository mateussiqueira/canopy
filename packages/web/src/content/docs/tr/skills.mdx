...
Ajan becerileri, Canopy'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
Canopy su konumlari tarar:

- Proje konfigurasyonu: `.Canopy/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/Canopy/skills/<name>/SKILL.md`
...
Proje yerel yollarinda Canopy, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.Canopy/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/Canopy/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.Canopy/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
Canopy, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`canopy.json.guard` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`canopy.json.guard` icinde):
