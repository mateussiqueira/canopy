...
Per diagnosticare problemi con Canopy, inizia controllando i log e i dati locali che salva su disco.
...
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\Canopy\log`

I file di log hanno nomi con timestamp (ad es. `2025-01-09T123456.log`) e vengono conservati i 10 file di log piu recenti.

Puoi impostare il livello di log con l'opzione a riga di comando `--log-level` per ottenere informazioni di debug piu dettagliate. Per esempio: `canopy --log-level DEBUG`.

---

## Archiviazione

Canopy salva i dati delle sessioni e altri dati dell'applicazione su disco in:

- **macOS/Linux**: `~/.local/share/Canopy/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\Canopy`
...
Canopy Desktop esegue in background un server locale di Canopy (il sidecar `Canopy-cli`). La maggior parte dei problemi e causata da un plugin che si comporta male, da una cache corrotta o da un'impostazione del server errata.
...
- Solo macOS: menu `Canopy` -> **Reload Webview** (utile se l'interfaccia e vuota o bloccata).
...
- **macOS/Linux** (installazioni vecchie): `~/.local/share/Canopy/canopy.json.guardc`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\Canopy\canopy.json.guardc`
...
Canopy puo anche caricare plugin locali dal disco. Spostali temporaneamente altrove (o rinomina la cartella) e riavvia l'app desktop:

- **Plugin globali**
  - **macOS/Linux**: `~/.config/Canopy/plugins/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\Canopy\plugins`
...
Se disabilitare i plugin non aiuta (o l'installazione di un plugin e bloccata), svuota la cache in modo che Canopy possa ricostruirla.

1. Chiudi completamente Canopy Desktop.
2. Elimina la directory della cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/Canopy`
- **Linux**: elimina `~/.cache/Canopy` (oppure esegui `rm -rf ~/.cache/Canopy`)
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.cache\Canopy`

3. Riavvia Canopy Desktop.

---

### Risolvi problemi di connessione al server

Canopy Desktop puo avviare il proprio server locale (predefinito) oppure connettersi a un URL server che hai configurato.
...
Se il tuo `canopy.json.guard(c)` contiene una sezione `server`, rimuovila temporaneamente e riavvia l'app desktop.
...
Su Windows, Canopy Desktop richiede **WebView2 Runtime** di Microsoft Edge. Se l'app si apre su una finestra vuota o non parte, installa/aggiorna WebView2 e riprova.

---

### Windows: problemi generali di prestazioni

Se riscontri prestazioni lente, problemi di accesso ai file o problemi del terminale su Windows, prova a usare [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL fornisce un ambiente Linux che funziona in modo piu fluido con le funzionalita di Canopy.

---

### Notifiche non visualizzate

Canopy Desktop mostra le notifiche di sistema solo quando:

- le notifiche sono abilitate per Canopy nelle impostazioni del sistema operativo, e
...
1. Chiudi Canopy Desktop.
2. Trova ed elimina questi file (si trovano nella directory dati dell'app Canopy Desktop):

- `Canopy.settings.dat` (desktop default server URL)
- `Canopy.global.dat` e `Canopy.workspace.*.dat` (stato dell'interfaccia come server/progetti recenti)
...
Se riscontri problemi con Canopy:
...
3. Assicurati di avere l'ultima versione con `canopy upgrade`
...
Per capire a quali modelli hai accesso, esegui `canopy models`
...
2. Se il problema persiste, prova a cancellare la configurazione salvata:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.local\share\Canopy`
...
Se incontri errori nelle chiamate API, potrebbe dipendere da pacchetti provider non aggiornati. Canopy installa dinamicamente i pacchetti provider (OpenAI, Anthropic, Google, ecc.) quando servono e li mette in cache localmente.
...
1. Svuota la cache dei pacchetti provider:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.cache\Canopy`

2. Riavvia Canopy per reinstallare i pacchetti provider piu recenti

Questo forzera Canopy a scaricare le versioni piu recenti dei pacchetti provider, cosa che spesso risolve problemi di compatibilita con parametri dei modelli e cambiamenti delle API.
...
Canopy rilevera se stai usando Wayland e preferira `wl-clipboard`; altrimenti provera a trovare gli strumenti per gli appunti nell'ordine: `xclip` e `xsel`.
