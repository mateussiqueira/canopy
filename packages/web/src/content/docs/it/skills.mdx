...
Le skill degli agenti permettono a Canopy di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
Canopy cerca in queste posizioni:

- Config di progetto: `.Canopy/skills/<name>/SKILL.md`
- Config globale: `~/.config/Canopy/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, Canopy risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.Canopy/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/Canopy/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.Canopy/skills/git-release/SKILL.md` cosi':
...
Canopy elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `canopy.json.guard`:
...
**Per agenti integrati** (in `canopy.json.guard`):
