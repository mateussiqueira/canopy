---
title: Plugin
description: Scrivi plugin per estendere Canopy.
---

I plugin ti permettono di estendere Canopy agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di Canopy.
...
- `.Canopy/plugins/` - plugin a livello progetto
- `~/.config/Canopy/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/Canopy/node_modules/`.
...
1. Config globale (`~/.config/Canopy/canopy.json.guard`)
2. Config di progetto (`canopy.json.guard`)
3. Directory plugin globale (`~/.config/Canopy/plugins/`)
4. Directory plugin di progetto (`.Canopy/plugins/`)
...
Canopy esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di Canopy per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere Canopy.
...
:::note
Se usi l'app desktop di Canopy, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a Canopy di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a Canopy:
...
L'helper `tool` crea uno strumento personalizzato che Canopy può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in Canopy insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://canopy.dev/docs/sdk) per i dettagli.
