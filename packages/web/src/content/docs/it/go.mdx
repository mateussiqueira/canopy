...
Canopy Go è un abbonamento a basso costo — **5 $ per il primo mese**, poi **10 $/mese** — che ti offre un accesso affidabile ai popolari modelli di programmazione aperti.

Go funziona come qualsiasi altro provider in Canopy. Ti abboni a Canopy Go e
...
Canopy Go ti dà accesso a questi modelli a **5 $ per il primo mese**, poi a **10 $/mese**.

---

## Come funziona

Canopy Go funziona come qualsiasi altro provider in Canopy.

1. Accedi a **<a href={console}>Canopy Zen</a>**, ti abboni a Go e
   copi la tua chiave API.
2. Esegui il comando `/connect` nella TUI, selezioni `Canopy Go` e incolli
   la tua chiave API.
3. Esegui `/models` nella TUI per vedere l'elenco dei modelli disponibili tramite Go.

:::note
Solo un membro per workspace può abbonarsi a Canopy Go.
:::
...
Canopy Go include i seguenti limiti:
...
Il [model id](/docs/config/#models) nella tua Canopy config
utilizza il formato `Canopy-go/<model-id>`. Ad esempio, per Kimi K2.7 Code, useresti
`Canopy-go/kimi-k2.7-code` nella tua configurazione.
...
4. Non avere **nessun lock-in**, permettendoti di utilizzare anche qualsiasi altro provider con Canopy.
