---
title: Supporto ACP
description: Usa Canopy in qualunque editor compatibile con ACP.
---

Canopy supporta [Agent Client Protocol](https://agentclientprotocol.com) (ACP), permettendoti di usarlo direttamente in editor e IDE compatibili.
...
Per usare Canopy via ACP, configura l'editor in modo che esegua il comando `canopy acp`.

Il comando avvia Canopy come sottoprocesso compatibile con ACP che comunica con l'editor via JSON-RPC su stdio.
...
Per aprirlo, seleziona il nuovo agente 'Canopy' nel selettore agenti di AI Chat.
...
Per usare Canopy come agente ACP in [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), aggiungi quanto segue alla config di Neovim:
...
Questa config imposta CodeCompanion per usare Canopy come agente ACP per la chat.
...
Canopy funziona via ACP come nel terminale. Tutte le funzionalita sono supportate:
:::note
...
- Server MCP configurati nella tua configurazione Canopy
