---
title: CLI
description: Opzioni e comandi della CLI di Canopy.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI di Canopy, per impostazione predefinita, avvia la [TUI](/docs/tui) quando viene eseguita senza argomenti.

```bash
Canopy
```

Ma accetta anche i comandi documentati in questa pagina. Questo ti permette di interagire con Canopy in modo programmatico.
...
Avvia l'interfaccia testuale di Canopy (interfaccia utente da terminale).
...
La CLI di Canopy include anche i seguenti comandi.

---

### agent

Gestisci gli agenti per Canopy.
...
Collega un terminale a un backend server di Canopy già in esecuzione avviato tramite i comandi `serve` o `web`.

```bash
Canopy attach [url]
```

Questo consente di usare la TUI con un backend Canopy remoto. Per esempio:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nome utente per l'autenticazione di base (predefinito: `OPENCODE_SERVER_USERNAME` o `Canopy`) |
...
Canopy si basa sull'elenco provider di [Models.dev](https://models.dev), quindi puoi usare `canopy auth login` per configurare le API key per qualunque provider tu voglia usare. Le credenziali vengono salvate in `~/.local/share/Canopy/auth.json`.

```bash
Canopy auth login
```

Quando Canopy si avvia, carica i provider dal file delle credenziali e, se presenti, anche eventuali key definite nell'ambiente o in un file `.env` nel progetto.
...
Usa `--refresh` per aggiornare l'elenco modelli in cache. È utile quando nuovi modelli vengono aggiunti a un provider e vuoi vederli in Canopy.
...
Esegui Canopy in modalità non interattiva passando un prompt direttamente.
...
Puoi anche collegarti a una istanza `canopy serve` già in esecuzione per evitare i cold boot dei server MCP ad ogni esecuzione:
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Attach a un server Canopy in esecuzione (es. http://localhost:4096)                           |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Password per l'autenticazione di base (predefinita: `OPENCODE_SERVER_PASSWORD`)                 |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nome utente per l'autenticazione di base (predefinito: `OPENCODE_SERVER_USERNAME` o `Canopy`) |
...
Avvia un server Canopy headless per accesso via API. Vedi le [server docs](/docs/server) per l'interfaccia HTTP completa.

```bash
Canopy serve
```

Avvia un server HTTP che espone accesso API alle funzionalità di Canopy senza la TUI. Imposta `OPENCODE_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `Canopy`).
...
Elenca tutte le sessioni Canopy.
...
Mostra statistiche di utilizzo token e costo per le sessioni Canopy.
...
Importa i dati di sessione da un file JSON o da un URL di condivisione Canopy.

```bash
Canopy import <file>
```

Puoi importare da un file locale o da un URL di condivisione Canopy.
...
Avvia un server Canopy headless con interfaccia web.

```bash
Canopy web
```

Avvia un server HTTP e apre un browser per accedere a Canopy tramite interfaccia web. Imposta `OPENCODE_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `Canopy`).
...
Disinstalla Canopy e rimuove tutti i file correlati.
...
Aggiorna Canopy all'ultima versione o a una versione specifica.
...
La CLI di Canopy accetta i seguenti flag globali.
...
Canopy può essere configurato tramite variabili d'ambiente.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Sovrascrive lo username basic auth (default `Canopy`)     |
