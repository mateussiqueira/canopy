...
Canopy si basa sull'elenco provider di [Models.dev](https://models.dev), quindi puoi usare `canopy auth login` per configurare le API key per qualunque provider tu voglia usare. Le credenziali vengono salvate in `~/.local/share/opencode/auth.json`.
...
Avvia un server HTTP e apre un browser per accedere a Canopy tramite interfaccia web. Imposta `OPENCODE_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `opencode`).
