...
1. Esegui `/connect` nella TUI, seleziona opencode e vai su [canopy.dev/auth](https://canopy.dev/auth).
...
La funzionalità [Sessions](https://docs.helicone.ai/features/sessions) di Helicone ti permette di raggruppare richieste LLM correlate. Usa il plugin [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) per loggare automaticamente ogni conversazione di Canopy come una sessione in Helicone.
...
:::tip
Ollama può auto-configurarsi per Canopy. Vedi la [documentazione di integrazione Ollama](https://docs.ollama.com/integrations/opencode) per dettagli.
:::
