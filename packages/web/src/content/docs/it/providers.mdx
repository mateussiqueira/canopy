---
title: Provider
description: Usa qualsiasi provider LLM in Canopy.
---

import config from "../../../../config.mjs"
export const console = config.console

Canopy usa [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) per supportare **oltre 75 provider LLM** e anche modelli locali.

Per aggiungere un provider devi:

1. Aggiungere le API key del provider con il comando `/connect`.
2. Configurare il provider nella configurazione di Canopy.
...
Canopy Zen e una lista di modelli forniti dal team Canopy, testati e
verificati per funzionare bene con Canopy. [Scopri di piu](/docs/zen).

:::tip
Se sei all'inizio, ti consigliamo di partire da Canopy Zen.
:::

1. Esegui `/connect` nella TUI, seleziona Canopy e vai su [canopy.dev/auth](https://canopy.dev/auth).
...
Funziona come qualsiasi altro provider in Canopy ed e totalmente opzionale.
...
Per usare Amazon Bedrock con Canopy:
...
2. **Configura l'autenticazione** usando uno dei seguenti metodi:
   ...
   ***
   ...
   Imposta una di queste variabili d'ambiente mentre esegui Canopy:
   ...
   ***
   ...
   Per configurazione specifica del progetto o persistente, usa `canopy.json.guard`:
...
:::info
L'uso dell'abbonamento Claude Pro/Max in Canopy non è ufficialmente supportato da [Anthropic](https://anthropic.com).
:::
...
Puoi configurare Canopy per utilizzare modelli locali tramite [Atomic Chat](https://atomic.chat), un'applicazione desktop che esegue LLM locali dietro un server API compatibile OpenAI (endpoint predefinito `http://127.0.0.1:1337/v1`).
...
2. Vai su [Azure AI Foundry](https://ai.azure.com/) e fai il deploy di un modello.
   ...
   :::note
   Il nome del deployment deve corrispondere al nome del modello affinché Canopy funzioni correttamente.
   :::
...
5. Esegui il comando `/models` per selezionare un modello.
   ...
   Puoi anche aggiungere modelli tramite la tua configurazione di Canopy.
...
salvare il token nello storage di auth di Canopy.
:::

##### GitLab Self-Hosted

:::note[nota di compliance]
Canopy usa un modello piccolo per alcuni task AI come generare il titolo della sessione.
È configurato per usare gpt-5-nano di default, ospitato da Zen. Per bloccare Canopy
a usare solo la tua istanza GitLab self-hosted, aggiungi quanto segue al tuo
file `canopy.json.guard`. Si raccomanda anche di disabilitare la condivisione sessioni.
...
Maggior documentazione sulla homepage di [Canopy-gitlab-auth](https://www.npmjs.com/package/Canopy-gitlab-auth).

##### Configurazione

Personalizza tramite `canopy.json.guard`:
...
Per usare il tuo abbonamento GitHub Copilot con Canopy:
...
Per usare Google Vertex AI con Canopy:
...
2. Imposta le variabili d'ambiente richieste:
   ...
   Impostale mentre esegui Canopy.
...
Nel caso vedessi una funzionalità o un modello di Helicone che non viene configurato automaticamente da Canopy, puoi sempre configurarlo tu stesso.
...
La funzionalità [Sessions](https://docs.helicone.ai/features/sessions) di Helicone ti permette di raggruppare richieste LLM correlate. Usa il plugin [Canopy-helicone-session](https://github.com/H2Shami/Canopy-helicone-session) per loggare automaticamente ogni conversazione di Canopy come una sessione in Helicone.
...
Il plugin inietta gli header `Helicone-Session-Id` e `Helicone-Session-Name` nelle tue richieste. Nella pagina Sessions di Helicone, vedrai ogni conversazione di Canopy elencata come sessione separata.
...
Puoi configurare Canopy per usare modelli locali tramite l'utility llama-server di [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puoi configurare Canopy per usare modelli locali tramite LM Studio.
...
Puoi configurare Canopy per usare modelli locali tramite Ollama.

:::tip
Ollama può auto-configurarsi per Canopy. Vedi la [documentazione di integrazione Ollama](https://docs.ollama.com/integrations/Canopy) per dettagli.
:::
...
Per usare Ollama Cloud con Canopy:
...
3. Copia la chiave API da usare in Canopy.
...
6. **Importante**: Prima di usare modelli cloud in Canopy, devi scaricare le informazioni del modello localmente:
...
Canopy Zen è una lista di modelli testati e verificati forniti dal team Canopy. [Scopri di più](/docs/zen).

1. Accedi a **<a href={console}>Canopy Zen</a>** e clicca **Create API Key**.

2. Esegui il comando `/connect` e cerca **Canopy Zen**.

   ```txt
   /connect
   ```

3. Inserisci la tua chiave API di Canopy.
...
4. Molti modelli OpenRouter sono precaricati di default, esegui il comando `/models` per selezionare quello che vuoi.
   ...
   Puoi anche aggiungere modelli addizionali tramite la tua configurazione di Canopy.
...
5. Puoi anche personalizzarli tramite la configurazione di Canopy. Ecco un esempio di specifica di un provider
...
Puoi anche personalizzare i modelli tramite la tua configurazione di Canopy. Ecco un esempio di come specificare l'ordine di routing dei provider.
...
:::tip
Puoi usare qualsiasi provider OpenAI-compatible con Canopy. La maggior parte dei provider AI moderni offre API OpenAI-compatible.
:::
...
4. Crea o aggiorna il tuo file `canopy.json.guard` nella directory del progetto:
...
I campi `limit` permettono a Canopy di capire quanto contesto rimane. I provider standard recuperano questi dati da models.dev automaticamente.
...
1. **Controlla il setup auth**: Esegui `canopy auth list` per vedere se le credenziali
...
2. Per provider personalizzati, controlla la config di Canopy e:
   - Assicurati che l'ID provider usato nel comando `/connect` corrisponda all'ID nella tua config Canopy.
