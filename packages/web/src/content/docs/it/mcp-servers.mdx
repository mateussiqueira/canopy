...
Puoi aggiungere strumenti esterni a Canopy usando il _Model Context Protocol_, o MCP. Canopy supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [Canopy Config](https://canopy.dev/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/Canopy`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
Canopy gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, Canopy:
...
Se il server richiede autenticazione, Canopy ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `canopy mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, Canopy memorizzera' i token in modo sicuro in `~/.local/share/Canopy/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in Canopy insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione Canopy come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare Canopy al tuo account Sentry.
