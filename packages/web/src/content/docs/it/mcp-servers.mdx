...
Se il server richiede autenticazione, Canopy ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `opencode mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, Canopy memorizzera' i token in modo sicuro in `~/.local/share/opencode/mcp-auth.json`.
