---
title: CLI
description: أوامر وخيارات CLI في Canopy.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

يبدأ CLI الخاص بـ Canopy افتراضيا واجهة [TUI](/docs/tui) عند تشغيله بدون أي معاملات.

```bash
Canopy
```

كما يدعم أيضا أوامر موثقة في هذه الصفحة، ما يتيح لك التفاعل مع Canopy برمجيا.
...
بدء واجهة المستخدم terminal (TUI) الخاصة بـ Canopy.
...
يتضمن CLI الخاص بـ Canopy أيضا الأوامر التالية.
...
إرفاق terminal بخادم الواجهة الخلفية لـ Canopy قيد التشغيل بالفعل، والذي تم تشغيله عبر الأمرين `serve` أو `web`.

```bash
Canopy attach [url]
```

يتيح ذلك استخدام واجهة TUI مع واجهة خلفية لـ Canopy تعمل عن بعد. على سبيل المثال:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`    | اسم مستخدم المصادقة الأساسية (الافتراضي `OPENCODE_SERVER_USERNAME` أو `Canopy`) |
...
يعتمد Canopy على قائمة المزودين في [Models.dev](https://models.dev)، لذا يمكنك استخدام `canopy auth login` لتهيئة مفاتيح API لأي مزود ترغب باستخدامه. يتم حفظ ذلك في `~/.local/share/Canopy/auth.json`.

```bash
Canopy auth login
```

عند تشغيل Canopy يقوم بتحميل المزودين من ملف بيانات الاعتماد، وكذلك أي مفاتيح معرّفة في متغيرات البيئة لديك أو في ملف `.env` ضمن مشروعك.
...
استخدم الراية `--refresh` لتحديث قائمة النماذج المخزنة مؤقتا. يفيد ذلك عند إضافة نماذج جديدة إلى مزود وتريد رؤيتها في Canopy.
...
تشغيل Canopy في وضع غير تفاعلي عبر تمرير موجه مباشرة.
...
يمكنك أيضا الإرفاق بمثيل `canopy serve` قيد التشغيل لتجنّب زمن الإقلاع البارد لخوادم MCP في كل تشغيل:
...
| <nobr><code>{"--attach"}</code></nobr>   |         | الإرفاق بخادم Canopy قيد التشغيل (مثل http://localhost:4096)                    |
| <nobr><code>{"--password"}</code></nobr> | `-p`    | كلمة مرور المصادقة الأساسية (الافتراضي `OPENCODE_SERVER_PASSWORD`)                |
| <nobr><code>{"--username"}</code></nobr> | `-u`    | اسم مستخدم المصادقة الأساسية (الافتراضي `OPENCODE_SERVER_USERNAME` أو `Canopy`) |
...
بدء خادم Canopy بدون واجهة للوصول عبر API. راجع [توثيق الخادم](/docs/server) لواجهة HTTP الكاملة.

```bash
Canopy serve
```

يشغّل هذا خادم HTTP يوفّر وصولا عبر API لوظائف Canopy دون واجهة TUI. اضبط `OPENCODE_SERVER_PASSWORD` لتفعيل مصادقة HTTP الأساسية (اسم المستخدم الافتراضي `Canopy`).
...
عرض إحصاءات استخدام الرموز والتكلفة لجلسات Canopy لديك.
...
استيراد بيانات الجلسة من ملف JSON أو رابط مشاركة Canopy.

```bash
Canopy import <file>
```

يمكنك الاستيراد من ملف محلي أو من رابط مشاركة Canopy.
...
بدء خادم Canopy بدون واجهة مع واجهة ويب.

```bash
Canopy web
```

يشغّل هذا خادم HTTP ويفتح متصفح ويب للوصول إلى Canopy عبر واجهة ويب. اضبط `OPENCODE_SERVER_PASSWORD` لتفعيل مصادقة HTTP الأساسية (اسم المستخدم الافتراضي `Canopy`).
...
إلغاء تثبيت Canopy وإزالة جميع الملفات ذات الصلة.
...
تحديث Canopy إلى أحدث إصدار أو إلى إصدار محدد.
...
يدعم سطر أوامر Canopy الخيارات العامة التالية.
...
يمكن تهيئة Canopy باستخدام متغيرات البيئة.
...
| `OPENCODE_SERVER_USERNAME`            | string  | تجاوز اسم مستخدم المصادقة الأساسية (الافتراضي `Canopy`) |
