...
مع Canopy يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم Canopy سمتنا `Canopy`.
...
- يريدون أن يطابق Canopy مظهر terminal لديهم
...
يدعم Canopy نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/Canopy/themes/*.json` أو `$XDG_CONFIG_HOME/Canopy/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.Canopy/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.Canopy/themes/*.json`
