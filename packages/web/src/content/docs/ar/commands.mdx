...
أنشئ الملف `.Canopy/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات Canopy أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ Canopy:
...
- على مستوى النظام: `~/.config/Canopy/commands/`
- لكل مشروع: `.Canopy/commands/`
...
يتضمن Canopy عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
