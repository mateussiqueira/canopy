...
description: أنشئ أدوات يمكن لـ LLM استدعاؤها في Canopy.
---

الأدوات المخصصة هي دوال تنشئها يمكن لـ LLM استدعاؤها أثناء المحادثات. تعمل جنبا إلى جنب مع [الأدوات المدمجة](/docs/tools) في Canopy مثل `read` و`write` و`bash`.
...
- محليا بوضعها في مجلد `.Canopy/tools/` داخل مشروعك.
- أو عالميا بوضعها في `~/.config/Canopy/tools/`.
