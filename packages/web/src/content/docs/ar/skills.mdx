...
تتيح مهارات الوكيل لـ Canopy اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث Canopy في هذه المواقع:

- إعدادات المشروع: `.Canopy/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/Canopy/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج Canopy صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.Canopy/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/Canopy/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.Canopy/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض Canopy المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `canopy.json.guard`:
...
**للوكلاء المدمجين** (في `canopy.json.guard`):
