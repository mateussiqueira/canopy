...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك Canopy المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `opencode mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن Canopy الرموز بشكل آمن في `~/.local/share/opencode/mcp-auth.json`.
