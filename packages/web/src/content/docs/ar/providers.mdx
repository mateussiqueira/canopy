---
title: المزوّدون
description: استخدام أي موفّر LLM في Canopy.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم Canopy ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات Canopy.
...
في `~/.local/share/Canopy/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات Canopy.
...
Canopy Zen هي قائمة نماذج يوفّرها فريق Canopy وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع Canopy. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع Canopy Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر Canopy، ثم انتقل إلى [canopy.dev/auth](https://canopy.dev/auth).
...
يعمل مثل أي مزوّد آخر في Canopy واستخدامه اختياري بالكامل.

---

## Canopy Go

Canopy Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق Canopy، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع Canopy.

1. شغّل الأمر `/connect` في TUI، واختر `Canopy Go`، ثم انتقل إلى [canopy.dev/auth](https://canopy.dev/zen).
...
لاستخدام Amazon Bedrock مع Canopy:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل Canopy:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `canopy.json.guard`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل Canopy غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين Canopy لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل Canopy بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات Canopy.
...
تخزين الرمز في مساحة تخزين مصادقة Canopy.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم Canopy نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر Canopy
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`canopy.json.guard` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [Canopy-gitlab-auth](https://www.npmjs.com/package/Canopy-gitlab-auth).

##### التهيئة

خصّص عبر `canopy.json.guard`:
...
لاستخدام اشتراك GitHub Copilot مع Canopy:
...
لاستخدام Google Vertex AI مع Canopy:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر Canopy، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [Canopy-helicone-session](https://github.com/H2Shami/Canopy-helicone-session) لتسجيل كل محادثة Canopy تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة Canopy مدرجة كجلسة مستقلة.
...
يمكنك ضبط Canopy لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط Canopy لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط Canopy لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع Canopy. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/Canopy) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع Canopy:
...
3. انسخ مفتاح API لاستخدامه في Canopy.
...
6. **مهم**: قبل استخدام نماذج السحابة في Canopy، يجب سحب معلومات النموذج محليا:
...
Canopy Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق Canopy. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>Canopy Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **Canopy Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ Canopy.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات Canopy.
...
5. يمكنك أيضا تخصيصها عبر إعدادات Canopy. إليك مثالا على تحديد مزوّد
...
يمكنك أيضا تخصيص النماذج عبر إعدادات Canopy. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع Canopy. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `canopy.json.guard` في دليل مشروعك:
...
تسمح حقول `limit` لـ Canopy بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `canopy auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات Canopy وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات Canopy.
