...
يشغّل الأمر `canopy serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل opencode استخدامها.
...
عيّن `OPENCODE_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `opencode`، أو عيّن `OPENCODE_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `canopy serve` و `canopy web`.
...
يمكنك تشغيل `canopy serve` لبدء خادم مستقل. إذا كانت واجهة opencode في terminal (TUI)
