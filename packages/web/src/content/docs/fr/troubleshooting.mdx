...
Pour déboguer les problèmes avec Canopy, commencez par vérifier les journaux et les données locales qu'il stocke sur le disque.
...
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\Canopy\log`
...
Vous pouvez définir le niveau de journalisation avec l'option de ligne de commande `--log-level` pour obtenir des informations de débogage plus détaillées. Par exemple, `canopy --log-level DEBUG`.

---

## Stockage

Canopy stocke les données de session et autres données d'application sur le disque à l'emplacement :

- **macOS/Linux** : `~/.local/share/Canopy/`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\Canopy`
...
Canopy Desktop exécute un serveur Canopy local (le side-car `Canopy-cli`) en arrière-plan. La plupart des problèmes sont causés par un plugin qui se comporte mal, un cache corrompu ou un mauvais paramètre du serveur.
...
- macOS uniquement : menu `Canopy` -> **Recharger la vue Web** (aide si l'interface utilisateur est vide/gelée).
...
- **macOS/Linux** (anciennes installations) : `~/.local/share/Canopy/canopy.json.guardc`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\Canopy\canopy.json.guardc`
...
Canopy peut également charger des plugins locaux à partir du disque. Écartez-les temporairement (ou renommez le dossier) et redémarrez l'application de bureau :

- **Plugins mondiaux**
  - **macOS/Linux** : `~/.config/Canopy/plugins/`
  - **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\Canopy\plugins`
...
Si la désactivation des plugins ne résout pas le problème (ou si l'installation d'un plugin est bloquée), videz le cache afin que Canopy puisse le reconstruire.

1. Quittez complètement Canopy Desktop.
2. Supprimez le répertoire cache :

- **macOS** : Finder -> `Cmd+Shift+G` -> coller `~/.cache/Canopy`
- **Linux** : supprimez `~/.cache/Canopy` (ou exécutez `rm -rf ~/.cache/Canopy`)
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.cache\Canopy`

3. Redémarrez le bureau Canopy.

---

### Résoudre les problèmes de connexion au serveur

Canopy Desktop peut soit démarrer son propre serveur local (par défaut), soit se connecter à un serveur URL que vous avez configuré.
...
Si votre `canopy.json.guard(c)` contient une section `server`, supprimez-la temporairement et redémarrez l'application de bureau.
...
Sur Windows, Canopy Desktop nécessite Microsoft Edge **WebView2 Runtime**. Si l'application s'ouvre sur une fenêtre vide ou ne démarre pas, installez/mettez à jour WebView2 et réessayez.

---

### Windows : problèmes de performances généraux

Si vous rencontrez des performances lentes, des problèmes d'accès aux fichiers ou des problèmes de terminal sur Windows, essayez d'utiliser [WSL (Windows Sous-système pour Linux)](/docs/windows-wsl). WSL fournit un environnement Linux qui fonctionne de manière plus transparente avec les fonctionnalités de Canopy.

---

### Les notifications ne s'affichent pas

Canopy Desktop affiche uniquement les notifications système lorsque :

- les notifications sont activées pour Canopy dans les paramètres de votre système d'exploitation, et
...
1. Quittez le bureau Canopy.
2. Recherchez et supprimez ces fichiers (ils se trouvent dans le répertoire de données de l'application Canopy Desktop) :

- `Canopy.settings.dat` (serveur de bureau par défaut URL)
- `Canopy.global.dat` et `Canopy.workspace.*.dat` (état de l'interface utilisateur comme les serveurs/projets récents)
...
Si vous rencontrez des problèmes avec Canopy :
...
3. Assurez-vous d'avoir la dernière version avec `canopy upgrade`
...
Pour déterminer à quels modèles vous avez accès, exécutez `canopy models`
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.local\share\Canopy`
...
Si vous rencontrez des erreurs d’appel API, cela peut être dû à des packages de fournisseurs obsolètes. Canopy installe dynamiquement les packages du fournisseur (OpenAI, Anthropic, Google, etc.) selon les besoins et les met en cache localement.
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.cache\Canopy`

2. Redémarrez Canopy pour réinstaller les derniers packages du fournisseur

Cela forcera Canopy à télécharger les versions les plus récentes des packages du fournisseur, ce qui résout souvent les problèmes de compatibilité avec les paramètres du modèle et les modifications de API.
...
Canopy détectera si vous utilisez Wayland et préférez `wl-clipboard`, sinon il essaiera de trouver les outils du presse-papiers dans l'ordre : `xclip` et `xsel`.
