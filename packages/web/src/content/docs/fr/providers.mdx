...
1. Exécutez la commande `/connect` dans le TUI, sélectionnez opencode et dirigez-vous vers [canopy.dev/auth](https://canopy.dev/auth).
...
La fonctionnalité [Sessions](https://docs.helicone.ai/features/sessions) d'Helicone vous permet de regrouper les requêtes LLM associées. Utilisez le plugin [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) pour enregistrer automatiquement chaque conversation Canopy en tant que session dans Helicone.
...
:::tip
Ollama peut se configurer automatiquement pour Canopy. Voir les [documents d'intégration Ollama](https://docs.ollama.com/integrations/opencode) pour plus de détails.
:::
