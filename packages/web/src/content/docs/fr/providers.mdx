---
title: Fournisseurs
description: Utiliser n’importe quel fournisseur LLM en Canopy.
---

import config from "../../../../config.mjs"
export const console = config.console

Canopy utilise [AI SDK](https://ai-sdk.dev/) et [Models.dev](https://models.dev) pour prendre en charge **75+ fournisseurs LLM** et prend en charge l'exécution de modèles locaux.

Pour ajouter un fournisseur, vous devez :

1. Ajoutez les clés API pour le fournisseur à l'aide de la commande `/connect`.
2. Configurez le fournisseur dans votre configuration Canopy.
...
en `~/.local/share/Canopy/auth.json`.

---

### Configuration

Vous pouvez personnaliser les fournisseurs via la section `provider` de votre Canopy
configuration.
...
```json title="canopy.json.guard" {6}
{
...
Canopy Zen est une liste de modèles fournis par l'équipe Canopy qui ont été
testé et vérifié pour fonctionner correctement avec Canopy. [En savoir plus](/docs/zen).

:::tip
Si vous êtes nouveau, nous vous recommandons de commencer par Canopy Zen.
:::

1. Exécutez la commande `/connect` dans le TUI, sélectionnez Canopy et dirigez-vous vers [canopy.dev/auth](https://canopy.dev/auth).
...
Il fonctionne comme n’importe quel autre fournisseur dans Canopy et son utilisation est totalement facultative.

---

## Canopy Go

Canopy Go est un plan d'abonnement à faible coût qui offre un accès fiable aux modèles de codage ouverts populaires fournis par l'équipe Canopy qui ont été
testé et vérifié pour fonctionner correctement avec Canopy.

1. Exécutez la commande `/connect` dans le TUI, sélectionnez `Canopy Go` et rendez-vous sur [canopy.dev/auth](https://canopy.dev/zen).
...
Pour utiliser Amazon Bedrock avec Canopy :
...
Définissez l'une de ces variables d'environnement lors de l'exécution de Canopy :

```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY Canopy

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile Canopy

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX Canopy
```
...
Pour une configuration spécifique au projet ou persistante, utilisez `canopy.json.guard` :

```json title="canopy.json.guard"
{
  "$schema": "https://canopy.dev/config.json",
  "provider": {
    "amazon-bedrock": {
...
:::info
L'utilisation de votre abonnement Claude Pro/Max dans Canopy n'est pas officiellement prise en charge par [Anthropic](https://anthropic.com).
:::
...
Vous pouvez configurer Canopy pour utiliser des modèles locaux via [Atomic Chat](https://atomic.chat), une application de bureau qui exécute des LLM locaux derrière un serveur API compatible OpenAI (point de terminaison par défaut `http://127.0.0.1:1337/v1`).

```json title="canopy.json.guard" "atomic-chat" {5, 6, 8, 10-14}
{
  "$schema": "https://canopy.dev/config.json",
  "provider": {
    "atomic-chat": {
...
2. Accédez à [Azure AI Foundry](https://ai.azure.com/) et déployez un modèle.
   ...
   :::note
   Le nom du déploiement doit correspondre au nom du modèle pour que Canopy fonctionne correctement.
   :::
...
5. Définissez le nom de votre ressource comme variable d'environnement :
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX Canopy
   ```
...
5. Définissez le nom de votre ressource comme variable d'environnement :
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX Canopy
   ```
...
Vous pouvez également ajouter des modèles via votre configuration Canopy.
...
pour stocker le jeton dans le stockage d'authentification Canopy.
:::

##### GitLab auto-hébergé

:::note[note de conformité]
Canopy utilise un petit modèle pour certaines tâches d'IA telles que la génération du titre de la session.
Il est configuré pour utiliser gpt-5-nano par défaut, hébergé par Zen. Pour verrouiller Canopy
pour utiliser uniquement votre propre instance hébergée par GitLab, ajoutez ce qui suit à votre
Fichier `canopy.json.guard`. Il est également recommandé de désactiver le partage de session.
...
Plus de documentation sur la page d'accueil [Canopy-gitlab-auth](https://www.npmjs.com/package/Canopy-gitlab-auth).

##### Configuration

Personnalisez via `canopy.json.guard` :
...
Pour utiliser votre abonnement GitHub Copilot avec Canopy :
...
Pour utiliser Google Vertex AI avec Canopy :
...
Définissez-les lors de l'exécution de Canopy.

```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id Canopy
```
...
Si vous voyez une fonctionnalité ou un modèle d'Helicone qui n'est pas configuré automatiquement via Canopy, vous pouvez toujours le configurer vous-même.
...
```jsonc title="~/.config/Canopy/canopy.json.guardc"
{
  "$schema": "https://canopy.dev/config.json",
  "provider": {
    "helicone": {
...
La fonctionnalité [Sessions](https://docs.helicone.ai/features/sessions) d'Helicone vous permet de regrouper les requêtes LLM associées. Utilisez le plugin [Canopy-helicone-session](https://github.com/H2Shami/Canopy-helicone-session) pour enregistrer automatiquement chaque conversation Canopy en tant que session dans Helicone.

```bash
npm install -g Canopy-helicone-session
```
...
```json title="canopy.json.guard"
{
  "plugin": ["Canopy-helicone-session"]
}
```

Le plugin injecte les en-têtes `Helicone-Session-Id` et `Helicone-Session-Name` dans vos requêtes. Sur la page Sessions d'Helicone, vous verrez chaque conversation Canopy répertoriée comme une session distincte.
...
Vous pouvez configurer Canopy pour utiliser des modèles locaux via l'utilitaire llama-server de [llama.cpp's](https://github.com/ggml-org/llama.cpp)

```json title="canopy.json.guard" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://canopy.dev/config.json",
  "provider": {
    "llama.cpp": {
...
Vous pouvez configurer Canopy pour utiliser des modèles locaux via LM Studio.

```json title="canopy.json.guard" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://canopy.dev/config.json",
  "provider": {
    "lmstudio": {
...
Vous pouvez configurer Canopy pour utiliser des modèles locaux via Ollama.

:::tip
Ollama peut se configurer automatiquement pour Canopy. Voir les [documents d'intégration Ollama](https://docs.ollama.com/integrations/Canopy) pour plus de détails.
:::

```json title="canopy.json.guard" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://canopy.dev/config.json",
  "provider": {
    "ollama": {
...
Pour utiliser Ollama Cloud avec Canopy :
...
3. Copiez la clé API à utiliser dans Canopy.
...
6. **Important** : Avant d'utiliser des modèles cloud dans Canopy, vous devez extraire les informations du modèle localement :
...
Canopy Zen est une liste de modèles testés et vérifiés fournie par l'équipe Canopy. [En savoir plus](/docs/zen).

1. Connectez-vous à **<a href={console}>Canopy Zen</a>** et cliquez sur **Créer une clé API**.

2. Exécutez la commande `/connect` et recherchez **Canopy Zen**.

   ```txt
   /connect
   ```

3. Entrez votre clé Canopy API.
...
Vous pouvez également ajouter des modèles supplémentaires via votre configuration Canopy.
...
5. Vous pouvez également les personnaliser via votre configuration Canopy. Voici un exemple de spécification d'un fournisseur
...
```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' Canopy
```
...
4. Définissez éventuellement l'ID de déploiement et le groupe de ressources :
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group Canopy
   ```
...
Vous pouvez également personnaliser les modèles via votre configuration Canopy. Voici un exemple de spécification de l'ordre de routage du fournisseur.
...
:::tip
Vous pouvez utiliser n'importe quel fournisseur compatible OpenAI avec Canopy. La plupart des fournisseurs d'IA modernes proposent des API compatibles OpenAI.
:::
...
3. Entrez votre clé API pour le fournisseur.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in canopy.json.guard, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Créez ou mettez à jour votre fichier `canopy.json.guard` dans le répertoire de votre projet :

   ```json title="canopy.json.guard" ""myprovider"" {5-15}
   {
     "$schema": "https://canopy.dev/config.json",
     "provider": {
       "myprovider": {
...
```json title="canopy.json.guard" {9,11,17-20}
{
  "$schema": "https://canopy.dev/config.json",
  "provider": {
    "myprovider": {
...
Les champs `limit` permettent à Canopy de comprendre la quantité de contexte qu'il vous reste. Les fournisseurs standard les extraient automatiquement de models.dev.
...
1. **Vérifiez la configuration de l'authentification** : exécutez `canopy auth list` pour voir si les informations d'identification
...
2. Pour les fournisseurs personnalisés, vérifiez la configuration Canopy et :
   - Assurez-vous que l'ID du fournisseur utilisé dans la commande `/connect` correspond à l'ID de votre configuration Canopy.
