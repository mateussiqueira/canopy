...
Avec Canopy, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, Canopy utilise notre propre thème `Canopy`.
...
Canopy est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que Canopy corresponde à l'apparence de leur terminal
...
Canopy prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/Canopy/themes/*.json` ou `$XDG_CONFIG_HOME/Canopy/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.Canopy/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.Canopy/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/Canopy/themes
vim ~/.config/Canopy/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .Canopy/themes
vim .Canopy/themes/my-theme.json
```
...
{
  "$schema": "https://canopy.dev/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
