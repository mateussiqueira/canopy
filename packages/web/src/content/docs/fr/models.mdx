...
Les fournisseurs les plus populaires sont préchargés par défaut. Si vous avez ajouté les informations d'identification d'un fournisseur via la commande `/connect`, elles seront disponibles lorsque vous démarrerez Canopy.
...
Voici plusieurs modèles qui fonctionnent bien avec Canopy, sans ordre particulier. (Cette liste n’est pas exhaustive et n’est pas nécessairement à jour) :
...
Pour définir l'un d'entre eux comme modèle par défaut, vous pouvez définir la clé `model` dans votre configuration Canopy.

```json title="canopy.json.guard" {3}
{
...
Ici, l'identifiant complet est `provider_id/model_id`. Par exemple, si vous utilisez [Canopy Zen](/docs/zen), vous utiliserez `canopy/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```jsonc title="canopy.json.guardc" {7-12,19-24}
{
  "$schema": "https://canopy.dev/config.json",
  "provider": {
    "openai": {
...
```jsonc title="canopy.json.guardc" {6-21}
{
  "$schema": "https://canopy.dev/config.json",
  "provider": {
    "Canopy": {
...
De nombreux modèles prennent en charge plusieurs variantes avec différentes configurations. Canopy est livré avec des variantes par défaut intégrées pour les fournisseurs populaires.

### Variantes intégrées

Canopy est livré avec des variantes par défaut pour de nombreux fournisseurs :
...
```jsonc title="canopy.json.guardc" {7-18}
{
  "$schema": "https://canopy.dev/config.json",
  "provider": {
    "openai": {
...
Lorsque Canopy démarre, il recherche les modèles dans l'ordre de priorité suivant :
...
2. La liste des modèles dans la configuration Canopy.
