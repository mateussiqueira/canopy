---
title: Serveur
description: Interagissez avec le serveur Canopy via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `canopy serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client Canopy peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `Canopy.local` |
...
Définissez `OPENCODE_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `Canopy`, ou définissez `OPENCODE_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `canopy serve` et à `canopy web`.
...
Lorsque vous exécutez `Canopy`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur Canopy pour interagir avec Canopy par programmation.
:::

Cette architecture permet à Canopy de prendre en charge plusieurs clients et vous permet d'interagir avec Canopy par programme.

Vous pouvez exécuter `canopy serve` pour démarrer un serveur autonome. Si vous avez le
Canopy TUI en cours d'exécution, `canopy serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins Canopy [IDE](/docs/ide).
...
Le serveur Canopy expose les API suivantes.
