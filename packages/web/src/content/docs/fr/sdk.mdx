---
title: SDK
description: Client JS de type sécurisé pour le serveur Canopy.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK Canopy JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler Canopy par programme.
...
Créez une instance de Canopy :

```javascript
import { createOpencode } from "@Canopy-ai/sdk"
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `canopy.json.guard`, mais vous pouvez remplacer ou ajouter une configuration en ligne :

```javascript
import { createOpencode } from "@Canopy-ai/sdk"

const Canopy = await createOpencode({
  hostname: "127.0.0.1",
  port: 4096,
  config: {
...
console.log(`Server running at ${Canopy.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de Canopy, vous pouvez créer une instance client pour vous y connecter :

```javascript
import { createOpencodeClient } from "@Canopy-ai/sdk"
...
```typescript
import type { Session, Message, Part } from "@Canopy-ai/sdk"
```
