---
title: Plugins
description: Écrivez vos propres plugins pour étendre Canopy.
---

Les plugins vous permettent d'étendre Canopy en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de Canopy.
...
- `.Canopy/plugins/` - Plugins au niveau du projet
- `~/.config/Canopy/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/Canopy/node_modules/`.
...
1. Configuration globale (`~/.config/Canopy/canopy.json.guard`)
2. Configuration du projet (`canopy.json.guard`)
3. Répertoire global des plugins (`~/.config/Canopy/plugins/`)
4. Répertoire des plugins du projet (`.Canopy/plugins/`)
...
Canopy exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK Canopy pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre Canopy.
...
:::note
Si vous utilisez l'application de bureau Canopy, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez Canopy de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à Canopy :
...
L'assistant `tool` crée un outil personnalisé que Canopy peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour Canopy aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://canopy.dev/docs/sdk) pour plus de détails.
