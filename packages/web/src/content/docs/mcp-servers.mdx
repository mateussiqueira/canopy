...
You can add external tools to Canopy using the _Model Context Protocol_, or MCP. Canopy supports both local and remote servers.
...
You can define MCP servers in your [Canopy Config](https://canopy.dev/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Canopy automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, Canopy will:
...
If the server requires authentication, Canopy will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `canopy mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, Canopy will store the tokens securely in `~/.local/share/opencode/mcp-auth.json`.
...
Your MCPs are available as tools in Canopy, alongside built-in tools. So you can manage them through the Canopy config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect Canopy to your Sentry account.
