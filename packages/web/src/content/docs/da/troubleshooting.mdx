...
For at fejlfinde problemer med Canopy, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\Canopy\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `canopy --log-level DEBUG`.

---

## Lagring

Canopy gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/Canopy/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\Canopy`
...
Canopy Desktop kører en lokal Canopy-server (`Canopy-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `Canopy`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/Canopy/canopy.json.guardc` (eller `~/.config/Canopy/canopy.json.guard`)
- **macOS/Linux** (ældre installationer): `~/.local/share/Canopy/canopy.json.guardc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\Canopy\canopy.json.guardc`
...
Canopy kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/Canopy/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\Canopy\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så Canopy kan genopbygge den.

1. Afslut Canopy Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/Canopy`
- **Linux**: slet `~/.cache/Canopy` (eller kør `rm -rf ~/.cache/Canopy`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\Canopy`

3. Start Canopy Desktop på ny.

---

### Løs problemer med serverforbindelse

Canopy Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `canopy.json.guard(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.
...
På Windows kræver Canopy Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i Canopy.

---

### Meddelelser vises ikke

Canopy Desktop viser kun systemvarsler når:

- varsler er aktiveret for Canopy i dine OS-indstillinger, og
...
1. Afslut Canopy Desktop.
2. Find og slet disse filer (de findes i Canopy Desktop-appens datamappe):

- `Canopy.settings.dat` (skrivebordsstandardserver URL)
- `Canopy.global.dat` og `Canopy.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med Canopy:
...
For at finde ud af hvilke modeller du har adgang til, kør `canopy models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\Canopy`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. Canopy installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\Canopy`

2. Start Canopy på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge Canopy til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
Canopy vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
