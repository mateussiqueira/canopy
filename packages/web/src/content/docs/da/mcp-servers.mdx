...
Hvis serveren kræver godkendelse, vil Canopy bede dig om at godkende, når du prøver at bruge den første gang. Hvis ikke, kan du [manually trigger the flow](#authenticating) med `opencode mcp auth <server-name>`.
...
Kommandoen `mcp auth` åbner din browser for godkendelse. Når du har godkendt, vil Canopy opbevare tokens sikkert i `~/.local/share/opencode/mcp-auth.json`.
