...
Med Canopy kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger Canopy vårt eget `Canopy`-tema.
...
Canopy kommer med flere innebygde temaer.
...
- Vil at Canopy skal matche terminalens utseende
...
Canopy støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/Canopy/themes/*.json` eller `$XDG_CONFIG_HOME/Canopy/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.Canopy/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.Canopy/themes/*.json`
