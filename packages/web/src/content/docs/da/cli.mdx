---
title: CLI
description: Canopy CLI muligheder og kommandoer.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Canopy CLI starter som standard [TUI](/docs/tui), når den kører uden nogen argumenter.

```bash
Canopy
```

Men den accepterer også kommandoer som dokumenteret på denne side. Dette giver dig mulighed for at interagere med Canopy programmatisk.
...
Start Canopy terminalbrugergrænsefladen.
...
Canopy CLI har også følgende kommandoer.

---

### agent

Administrer agenter i Canopy.
...
Tilslut en terminal til en allerede kørende Canopy backend-server startet via `serve` eller `web` kommandoer.

```bash
Canopy attach [url]
```

Dette gør det muligt at bruge TUI med en ekstern Canopy backend. For eksempel:
...
| <nobr><code>{"--username"}</code></nobr> | `-u` | Brugernavn til basic auth (standard: `OPENCODE_SERVER_USERNAME` eller `Canopy`) |
...
Canopy drives af udbyderlisten på [Models.dev](https://models.dev), så du kan bruge `canopy auth login` til at konfigurere API-nøgler for enhver udbyder, du vil bruge. Dette er gemt i `~/.local/share/Canopy/auth.json`.

```bash
Canopy auth login
```

Når Canopy starter op, indlæser den udbyderne fra legitimationsfilen. Og hvis der er nogen nøgler defineret i dine miljøer eller en `.env` fil i dit projekt.
...
Brug flaget `--refresh` til at opdatere den cachelagrede modelliste. Dette er nyttigt, når nye modeller er blevet tilføjet til en udbyder, og du vil se dem i Canopy.
...
Kør Canopy i ikke-interaktiv tilstand ved at sende en prompt direkte.
...
Du kan også vedhæfte til en kørende `canopy serve`-instans for at undgå MCP serverens kolde opstartstider ved hver kørsel:
...
| <nobr><code>{"--attach"}</code></nobr>   |      | Tilslut til en kørende Canopy-server (f.eks. http://localhost:4096)               |
| <nobr><code>{"--password"}</code></nobr> | `-p` | Adgangskode til basic auth (standard: `OPENCODE_SERVER_PASSWORD`)                   |
| <nobr><code>{"--username"}</code></nobr> | `-u` | Brugernavn til basic auth (standard: `OPENCODE_SERVER_USERNAME` eller `Canopy`)   |
...
Start en hovedløs Canopy-server til API-adgang. Tjek [server docs](/docs/server) for den fulde HTTP-grænseflade.

```bash
Canopy serve
```

Dette starter en HTTP-server, der giver API-adgang til Canopy-funktionalitet uden TUI-grænsefladen. Indstil `OPENCODE_SERVER_PASSWORD` for at aktivere HTTP grundlæggende godkendelse (brugernavn er standard til `Canopy`).
...
Administrator Canopy sessionsholder.
...
Liste over alle Canopy sessioner.
...
Vis tokenbrug og omkostningsstatistikker for dine Canopy-sessioner.
...
Importer sessionsdata fra en JSON fil eller Canopy del URL.

```bash
Canopy import <file>
```

Du kan importere fra en lokal fil eller en Canopy share URL.
...
Start en hovedløs Canopy-server med en webgrænseflade.

```bash
Canopy web
```

Dette starter en HTTP-server og åbner en webbrowser for at få adgang til Canopy via en webgrænseflade. Indstil `OPENCODE_SERVER_PASSWORD` for at aktivere HTTP grundlæggende godkendelse (brugernavn er standard til `Canopy`).
...
Afinstaller Canopy og fjern alle relaterede filer.
...
Opdaterer Canopy til den nyeste version eller en specifik version.
...
Canopy CLI tager følgende globale flag.
...
Canopy kan konfigureres ved hjælp af miljøvariabler.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Tilsidesæt grundlæggende godkendelsesbrugernavn (standard `Canopy`) |
