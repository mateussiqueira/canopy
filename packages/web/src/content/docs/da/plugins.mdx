---
title: Plugins
description: Skriv dine egne plugins for at udvide Canopy.
---

Plugins giver dig mulighed for at udvide Canopy ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre Canopys standardadfærd.
...
- `.Canopy/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/Canopy/node_modules/`.
...
1. Global konfiguration (`~/.config/Canopy/canopy.json.guard`)
2. Projektkonfiguration (`canopy.json.guard`)
3. Global plugin-mappe (`~/.config/Canopy/plugins/`)
4. Projekt plugin bibliotek (`.Canopy/plugins/`)
...
Canopy kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En Canopy SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide Canopy.
...
:::note
Hvis du bruger Canopy desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå Canopy i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til Canopy:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som Canopy kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for Canopy sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://canopy.dev/docs/sdk) for detaljer.
