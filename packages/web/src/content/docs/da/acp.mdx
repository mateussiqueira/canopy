...
description: Brug Canopy i enhver ACP-kompatibel editor.
---

Canopy understøtter [Agent Client Protocol](https://agentclientprotocol.com) eller (ACP), så du kan bruge det direkte i kompatible editorer og IDE'er.
...
For at bruge Canopy via ACP, konfigurer din editor til at køre kommandoen `canopy acp`.

Kommandoen starter Canopy som en ACP-kompatibel underproces, der kommunikerer med din editor over JSON-RPC via stdio.
...
For at åbne den skal du bruge den nye 'Canopy'-agent i AI Chat-agentvælgeren.
...
For at bruge Canopy som en ACP-agent i [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), skal du tilføje følgende til din Neovim-konfiguration:
...
Denne konfiguration sætter CodeCompanion til at bruge Canopy som ACP-agent til chat.
...
Canopy fungerer på samme måde via ACP som i terminalen. Alle funktioner understøtter:
...
- MCP-servere konfigureret i din Canopy-konfiguration
