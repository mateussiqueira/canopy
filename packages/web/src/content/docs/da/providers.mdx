---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i Canopy.
---

import config from "../../../../config.mjs"
export const console = config.console

Canopy bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i Canopy-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/Canopy/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i Canopy-konfigurationen.
...
Canopy Zen er en liste over modeller leveret af Canopy-teamet, som er blevet testet og verificeret til at fungere godt med Canopy. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med Canopy Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg Canopy og gå til [canopy.dev/auth](https://canopy.dev/auth).
...
Det fungerer som alle andre udbydere i Canopy og er helt valgfrit at bruge.

---

## Canopy Go

Canopy Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af Canopy-teamet, som er testet og verificeret til at fungere godt med Canopy.

1. Kør kommandoen `/connect` i TUI, vælg `Canopy Go`, og gå til [canopy.dev/auth](https://canopy.dev/zen).
...
Sådan bruger du Amazon Bedrock med Canopy:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører Canopy:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `canopy.json.guard`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i Canopy understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere Canopy til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at Canopy skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem Canopy-konfigurationen.
...
gemme tokenet i Canopy auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
Canopy bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse Canopy
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`canopy.json.guard` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [Canopy-gitlab-auth](https://www.npmjs.com/package/Canopy-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `canopy.json.guard`:
...
Sådan bruger du GitHub Copilot-abonnementet med Canopy:
...
Sådan bruger du Google Vertex AI med Canopy:
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem Canopy, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [Canopy-helicone-session](https://github.com/H2Shami/Canopy-helicone-session) plugin for automatisk at logge hver Canopy-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver Canopy-samtale opført som en separat session.
...
Du kan konfigurere Canopy til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere Canopy til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere Canopy til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for Canopy. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/Canopy) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med Canopy:
...
3. Kopier API-nøglen til brug i Canopy.
...
6. **Vigtigt**: Før du bruger skymodeller i Canopy, skal du hente modelinformationen lokalt:
...
Canopy Zen er en liste over testede og verificerede modeller leveret af Canopy-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>Canopy Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **Canopy Zen**.

   ```txt
   /connect
   ```

3. Indtast Canopy API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem Canopy-konfigurationen.
...
5. Du kan også tilpasse dem gennem Canopy-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Canopy:
...
Du kan også tilpasse modeller gennem Canopy-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med Canopy. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `canopy.json.guard`-filen i projektkataloget:
...
`limit`-felterne lader Canopy forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `canopy auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek Canopy-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i Canopy-konfigurationen.
