...
The `canopy serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an opencode client can use.
...
Set `OPENCODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `opencode`, or set `OPENCODE_SERVER_USERNAME` to override it. This applies to both `canopy serve` and `canopy web`.
...
You can run `canopy serve` to start a standalone server. If you have the
opencode TUI running, `canopy serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Canopy [IDE](/docs/ide) plugins.
