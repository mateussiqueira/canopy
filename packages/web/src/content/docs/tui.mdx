---
title: TUI
description: Using the Canopy terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Canopy provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running Canopy starts the TUI for the current directory.
...
When using the Canopy TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to Canopy. Allows you to select from available providers and add their API keys.
...
- `leader_timeout` - Controls how long Canopy waits after the leader key. Defaults to `2000`.
...
- `sound_pack` - Sound pack ID to use. Defaults to `canopy.default`.
