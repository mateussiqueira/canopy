...
description: Cree herramientas que LLM pueda llamar en Canopy.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de Canopy como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.Canopy/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/Canopy/tools/`.
...
```ts title=".Canopy/tools/database.ts" {1}
import { tool } from "@Canopy-ai/plugin"
...
```ts title=".Canopy/tools/project.ts" {8}
import { tool } from "@Canopy-ai/plugin"
...
```python title=".Canopy/tools/add.py"
import sys
...
```ts title=".Canopy/tools/python-add.ts" {10}
import { tool } from "@Canopy-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".Canopy/tools/add.py")
