---
title: Complementos
description: Escriba sus propios complementos para extender Canopy.
---

Los complementos le permiten extender Canopy al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de Canopy.
...
- `.Canopy/plugins/` - Complementos a nivel de proyecto
- `~/.config/Canopy/plugins/` - Complementos globales
...
```json title="canopy.json.guard"
{
  ...
  "plugin": ["Canopy-helicone-session", "Canopy-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/Canopy/node_modules/`.
...
1. Configuración global (`~/.config/Canopy/canopy.json.guard`)
2. Configuración del proyecto (`canopy.json.guard`)
3. Directorio global de complementos (`~/.config/Canopy/plugins/`)
4. Directorio de complementos del proyecto (`.Canopy/plugins/`)
...
```json title=".Canopy/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
Canopy ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".Canopy/plugins/my-plugin.ts"
...
```js title=".Canopy/plugins/example.js"
...
- `client`: Un cliente SDK Canopy para interactuar con la IA.
...
import type { Plugin } from "@Canopy-ai/plugin"
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar Canopy.
...
```js title=".Canopy/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "Canopy"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio Canopy, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que Canopy lea archivos `.env`:

```javascript title=".Canopy/plugins/env-protection.js"
...
```javascript title=".Canopy/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a Canopy:

```ts title=".Canopy/plugins/custom-tools.ts"
import { type Plugin, tool } from "@Canopy-ai/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que Canopy puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para Canopy junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://canopy.dev/docs/sdk) para obtener más detalles.
...
```ts title=".Canopy/plugins/compaction.ts"
import type { Plugin } from "@Canopy-ai/plugin"
...
```ts title=".Canopy/plugins/custom-compaction.ts"
import type { Plugin } from "@Canopy-ai/plugin"
