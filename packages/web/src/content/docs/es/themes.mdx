...
Con Canopy puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, Canopy usa nuestro propio tema `Canopy`.
...
Canopy viene con varios temas integrados.
...
- Quiere que Canopy coincida con la apariencia de su terminal
...
Canopy admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/Canopy/themes/*.json` o `$XDG_CONFIG_HOME/Canopy/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.Canopy/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.Canopy/themes/*.json`
