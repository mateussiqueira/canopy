---
title: Servidor
description: Interactuar con el servidor Canopy a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `canopy serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente Canopy puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `Canopy.local` |
...
Configure `OPENCODE_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `Canopy`, o configure `OPENCODE_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `canopy serve` como a `canopy web`.
...
Cuando ejecuta `Canopy`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor Canopy para interactuar con Canopy mediante programación.
:::

Esta arquitectura permite que Canopy admita múltiples clientes y le permite interactuar con Canopy mediante programación.

Puede ejecutar `canopy serve` para iniciar un servidor independiente. Si tienes el
Canopy TUI ejecutándose, `canopy serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos Canopy [IDE](/docs/ide).
...
El servidor Canopy expone las siguientes API.
