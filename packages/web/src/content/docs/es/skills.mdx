...
Las habilidades del agente permiten a Canopy descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
Canopy busca estas ubicaciones:

- Configuración del proyecto: `.Canopy/skills/<name>/SKILL.md`
- Configuración global: `~/.config/Canopy/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, Canopy sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.Canopy/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/Canopy/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.Canopy/skills/git-release/SKILL.md` así:
...
Canopy enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `canopy.json.guard`:
...
**Para agentes integrados** (en `canopy.json.guard`):
