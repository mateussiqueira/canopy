...
Para depurar problemas con Canopy, comience verificando los registros y los datos locales que almacena en el disco.
...
- **macOS/Linux**: `~/.local/share/Canopy/log/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\Canopy\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `canopy --log-level DEBUG`.

---

## Almacenamiento

Canopy almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/Canopy/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\Canopy`
...
Canopy Desktop ejecuta un servidor Canopy local (el sidecar `Canopy-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `Canopy` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/Canopy/canopy.json.guardc` (o `~/.config/Canopy/canopy.json.guard`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/Canopy/canopy.json.guardc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\Canopy\canopy.json.guardc`
...
Canopy también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:

- **Complementos globales**
  - **macOS/Linux**: `~/.config/Canopy/plugins/`
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\Canopy\plugins`
- **Complementos de proyecto** (solo si usas la configuración por proyecto)
  - `<your-project>/.Canopy/plugins/`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que Canopy pueda reconstruirlo.

1. Salga de Canopy Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/Canopy`
- **Linux**: eliminar `~/.cache/Canopy` (o ejecutar `rm -rf ~/.cache/Canopy`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\Canopy`

3. Reinicie el escritorio Canopy.

---

### Solucionar problemas de conexión del servidor

Canopy Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `canopy.json.guard(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.
...
En Windows, el escritorio Canopy requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de Canopy.

---

### Notificaciones que no se muestran

Canopy Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para Canopy en la configuración de su sistema operativo, y
...
1. Salga del escritorio Canopy.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio Canopy):

- `Canopy.settings.dat` (URL del servidor predeterminado de escritorio)
- `Canopy.global.dat` y `Canopy.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
1. **Informar problemas el GitHub**
   ...
   [**github.com/mateussiqueira/canopy/issues**](https://github.com/mateussiqueira/canopy/issues)
...
2. **Únete a nuestro Discord**
   ...
   [**canopy.dev/discord**](https://canopy.dev/discord)
...
3. Asegúrese de tener la última versión con `canopy upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `canopy models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\Canopy`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. Canopy instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\Canopy`

2. Reinicie Canopy para reinstalar los paquetes del proveedor más recientes.

Esto obligará a Canopy a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
Canopy detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
