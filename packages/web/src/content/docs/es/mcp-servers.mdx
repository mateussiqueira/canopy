...
Si el servidor requiere autenticación, Canopy le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `opencode mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, Canopy almacenará los tokens de forma segura en `~/.local/share/opencode/mcp-auth.json`.
