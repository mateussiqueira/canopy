---
title: CLI
description: Canopy CLI options and commands.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

The Canopy CLI by default starts the [TUI](/docs/tui) when run without any arguments.

```bash
opencode
```

But it also accepts commands as documented on this page. This allows you to interact with Canopy programmatically.
...
Start the Canopy terminal user interface.
...
The Canopy CLI also has the following commands.

---

### agent

Manage agents for Canopy.
...
Attach a terminal to an already running Canopy backend server started via `serve` or `web` commands.

```bash
opencode attach [url]
```

This allows using the TUI with a remote Canopy backend. For example:
...
Canopy is powered by the provider list at [Models.dev](https://models.dev), so you can use `canopy auth login` to configure API keys for any provider you'd like to use. This is stored in `~/.local/share/opencode/auth.json`.

```bash
opencode auth login
```

When Canopy starts up it loads the providers from the credentials file. And if there are any keys defined in your environments or a `.env` file in your project.
...
Use the `--refresh` flag to update the cached model list. This is useful when new models have been added to a provider and you want to see them in Canopy.
...
You can also attach to a running `canopy serve` instance to avoid MCP server cold boot times on every run:
...
Start a headless Canopy server for API access. Check out the [server docs](/docs/server) for the full HTTP interface.
...
Show token usage and cost statistics for your Canopy sessions.
...
Import session data from a JSON file or Canopy share URL.

```bash
opencode import <file>
```

You can import from a local file or an Canopy share URL.
...
Start a headless Canopy server with a web interface.

```bash
opencode web
```

This starts an HTTP server and opens a web browser to access Canopy through a web interface. Set `OPENCODE_SERVER_PASSWORD` to enable HTTP basic auth (username defaults to `opencode`).
...
Fetch and checkout a GitHub PR branch, then run Canopy.
...
Uninstall Canopy and remove all related files.
...
Canopy can be configured using environment variables.
