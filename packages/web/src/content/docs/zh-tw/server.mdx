...
`canopy serve` 指令執行一個無介面的 HTTP 伺服器，暴露一個 OpenAPI 端點供 opencode 客戶端使用。
...
設定 `OPENCODE_SERVER_PASSWORD` 以使用 HTTP 基本認證保護伺服器。使用者名稱預設為 `opencode`，也可以設定 `OPENCODE_SERVER_USERNAME` 來覆蓋它。這適用於 `canopy serve` 和 `canopy web`。
...
你可以執行 `canopy serve` 來啟動一個獨立的伺服器。如果你已經在執行 opencode TUI，`canopy serve` 會啟動一個新的伺服器。
