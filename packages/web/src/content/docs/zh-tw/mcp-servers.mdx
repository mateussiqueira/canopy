...
如果伺服器需要身分驗證，Canopy 會在您首次使用時提示您進行認證。您也可以使用 `opencode mcp auth <server-name>` [手動觸發認證流程](#authenticating)。
...
`mcp auth` 指令會開啟瀏覽器進行授權。授權完成後，Canopy 會將 Token 安全地儲存在 `~/.local/share/opencode/mcp-auth.json` 中。
