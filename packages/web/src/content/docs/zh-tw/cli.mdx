...
Canopy 基於 [Models.dev](https://models.dev) 的供應商列表運作，因此您可以使用 `canopy auth login` 為任何想要使用的供應商設定 API 金鑰。金鑰儲存在 `~/.local/share/opencode/auth.json` 中。
...
此指令啟動一個 HTTP 伺服器，提供對 Canopy 功能的 API 存取，無需 TUI 介面。設定 `OPENCODE_SERVER_PASSWORD` 可啟用 HTTP 基本認證（使用者名稱預設為 `opencode`）。
...
此指令啟動一個 HTTP 伺服器並開啟瀏覽器，透過 Web 介面存取 Canopy。設定 `OPENCODE_SERVER_PASSWORD` 可啟用 HTTP 基本認證（使用者名稱預設為 `opencode`）。
