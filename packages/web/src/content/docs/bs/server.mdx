...
Komanda `canopy serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji opencode klijent moze koristiti.
...
Postavite `OPENCODE_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `opencode`, ili postavite `OPENCODE_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `canopy serve` i za `canopy web`.
...
Mozete pokrenuti `canopy serve` da startate standalone server. Ako je opencode TUI vec pokrenut, `canopy serve` ce pokrenuti novi server.
