---
title: Server
description: Komunicirajte s Canopy serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `canopy serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji Canopy klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `Canopy.local` |
...
Postavite `OPENCODE_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `Canopy`, ili postavite `OPENCODE_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `canopy serve` i za `canopy web`.
...
Kada pokrenete `Canopy`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite Canopy server za programsku interakciju sa Canopy-om.
:::

Ova arhitektura omogucava Canopy podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `canopy serve` da startate standalone server. Ako je Canopy TUI vec pokrenut, `canopy serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste Canopy [IDE](/docs/ide) pluginovi.
...
Canopy server izlaže sljedece API-je.
