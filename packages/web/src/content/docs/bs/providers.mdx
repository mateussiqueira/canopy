---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u Canopy.
---

import config from "../../../../config.mjs"
export const console = config.console

Canopy koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj Canopy konfiguraciji.
...
u `~/.local/share/Canopy/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem Canopy
config.
...
Canopy Zen je lista modela koje je obezbedio Canopy tim koji su bili
testirano i potvrđeno da dobro radi sa Canopy. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa Canopy Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite Canopy i idite na [canopy.dev/auth](https://canopy.dev/auth).
...
Radi kao i svaki drugi provajder u Canopy i potpuno je opcionalan za korištenje.

---

## Canopy Go

Canopy Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža Canopy tim i koji su testirani i verificirani da dobro rade s Canopy-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `Canopy Go` i idite na [canopy.dev/auth](https://canopy.dev/zen).
...
Da biste koristili Amazon Bedrock s Canopy:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete Canopy:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `canopy.json.guard`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u Canopy.
:::
...
Možete konfigurirati Canopy za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi Canopy ispravno radio.
   :::
...
Također možete dodati modele kroz svoju Canopy konfiguraciju.
...
da pohrani token u Canopy auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
Canopy koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate Canopy
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`canopy.json.guard` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [Canopy-gitlab-auth](https://www.npmjs.com/package/Canopy-gitlab-auth).

##### Konfiguracija

Prilagodite putem `canopy.json.guard`:
...
Da biste koristili svoju GitHub Copilot pretplatu s Canopy:
...
Za korištenje Google Vertex AI s Canopy:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće Canopy.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem Canopya, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [Canopy-helicone-session](https://github.com/H2Shami/Canopy-helicone-session) da automatski prijavite svaki Canopy razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki Canopy razgovor naveden kao zasebna sesija.
...
Možete konfigurirati Canopy za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati Canopy za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati Canopy za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za Canopy. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/Canopy) za detalje.
:::
...
Da biste koristili Ollama Cloud s Canopy:
...
3. Kopirajte API ključ za korištenje u Canopy.
...
6. **Važno**: Prije upotrebe modela oblaka u Canopy, morate lokalno povući informacije o modelu:
...
Canopy Zen je lista testiranih i verifikovanih modela koju je obezbedio Canopy tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>Canopy Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **Canopy Zen**.

```txt
   /connect
```

3. Unesite svoj Canopy API ključ.
...
Također možete dodati dodatne modele putem vaše Canopy konfiguracije.
...
5. Također ih možete prilagoditi putem vaše Canopy konfiguracije. Evo primjera navođenja provajdera
...
Također možete prilagoditi modele kroz svoju Canopy konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s Canopy-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `canopy.json.guard` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju Canopy da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `canopy auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite Canopy konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj Canopy konfiguraciji.
