...
Da biste otklonili probleme s Canopy, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\Canopy\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `canopy --log-level DEBUG`.

---

## Pohrana

Canopy pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/Canopy/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\Canopy`
...
Canopy Desktop pokreće lokalni Canopy server (`Canopy-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `Canopy` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/Canopy/canopy.json.guardc` (ili `~/.config/Canopy/canopy.json.guard`)
- **macOS/Linux** (starije instalacije): `~/.local/share/Canopy/canopy.json.guardc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\Canopy\canopy.json.guardc`
...
Canopy također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/Canopy/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\Canopy\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga Canopy mogao ponovo izgraditi.

1. Potpuno zatvorite Canopy Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/Canopy`
- **Linux**: obrišite `~/.cache/Canopy` (ili pokrenite `rm -rf ~/.cache/Canopy`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\Canopy`

3. Ponovo pokrenite Canopy Desktop.

---

### Rješavanje problema sa vezom na serveru

Canopy Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `canopy.json.guard(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.
...
Na Windows-u, Canopy Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa Canopy karakteristikama.

---

### Obavještenja se ne prikazuju

Canopy Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za Canopy u postavkama vašeg OS-a, i
...
1. Zatvorite Canopy Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka Canopy Desktop aplikacije):

- `Canopy.settings.dat` (URL zadanog servera za desktop)
- `Canopy.global.dat` i `Canopy.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Da saznate kojim modelima imate pristup, pokrenite `canopy models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\Canopy`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. Canopy dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\Canopy`

2. Ponovo pokrenite Canopy da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti Canopy da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
Canopy će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
