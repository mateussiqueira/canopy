---
title: Dodaci
description: Napišite vlastite dodatke za proširenje Canopy.
---

Dodaci vam omogućavaju da proširite Canopy spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja Canopy.
...
- `.Canopy/plugins/` - Dodaci na nivou projekta
- `~/.config/Canopy/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/Canopy/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/Canopy/canopy.json.guard`)
2. Konfiguracija projekta (`canopy.json.guard`)
3. Globalni direktorij dodataka (`~/.config/Canopy/plugins/`)
4. Direktorij dodataka projekta (`.Canopy/plugins/`)
...
Canopy pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje Canopy.
...
Spriječite Canopy da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u Canopy:
...
Pomoćnik `tool` kreira prilagođeni alat koji Canopy može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za Canopy zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://canopy.dev/docs/sdk) za detalje.
