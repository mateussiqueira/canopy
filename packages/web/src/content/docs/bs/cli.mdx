---
title: CLI
description: Canopy CLI opcije i naredbe.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Canopy CLI po defaultu pokreće [TUI](/docs/tui) kada se pokrene bez ikakvih argumenata.

```bash
Canopy
```

Ali takođe prihvata naredbe kao što je dokumentovano na ovoj stranici. Ovo vam omogućava programsku interakciju sa Canopy.
...
Pokrenite Canopy terminalski korisnički interfejs.
...
Canopy CLI takođe ima sljedeće naredbe.

---

### agent

Upravljajte Canopy agentima.
...
Priključite terminal na već pokrenut Canopy backend server pokrenut putem `serve` ili `web` naredbi.

```bash
Canopy attach [url]
```

Ovo omogućava korištenje TUI-ja sa udaljenim Canopy backend-om. Na primjer:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`   | Korisničko ime za osnovnu autentifikaciju (zadano: `OPENCODE_SERVER_USERNAME` ili `Canopy`) |
...
Canopy pokreće lista provajdera na [Models.dev](https://models.dev), tako da možete koristiti `canopy auth login` da konfigurirate API ključeve za bilo kojeg provajdera kojeg želite koristiti. Ovo je pohranjeno u `~/.local/share/Canopy/auth.json`.

```bash
Canopy auth login
```

Kada se Canopy pokrene, učitava dobavljače iz datoteke vjerodajnica. I ako postoje neki ključevi definirani u vašim okruženjima ili `.env` fajl u vašem projektu.
...
Koristite `--refresh` zastavicu da ažurirate keširanu listu modela. Ovo je korisno kada su novi modeli dodani provajderu i želite da ih vidite u Canopy.
...
Pokrenite Canopy u neinteraktivnom modu tako što ćete direktno proslijediti prompt.
...
Također možete priključiti pokrenutu `canopy serve` instancu kako biste izbjegli vrijeme hladnog pokretanja MCP servera pri svakom pokretanju:
...
| <nobr><code>{"--attach"}</code></nobr>   |        | Priključite na pokrenuti Canopy server (npr. http://localhost:4096)                         |
| <nobr><code>{"--password"}</code></nobr> | `-p`   | Lozinka za osnovnu autentifikaciju (zadano: `OPENCODE_SERVER_PASSWORD`)                       |
| <nobr><code>{"--username"}</code></nobr> | `-u`   | Korisničko ime za osnovnu autentifikaciju (zadano: `OPENCODE_SERVER_USERNAME` ili `Canopy`) |
...
Pokrenite Canopy headless server za API pristup. Pogledajte [server docs](/docs/server) za kompletan HTTP interfejs.

```bash
Canopy serve
```

Ovo pokreće HTTP server koji pruža API pristup funkcionalnosti Canopy-a bez TUI interfejsa. Postavite `OPENCODE_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `Canopy`).
...
Prikaži statistiku upotrebe tokena i troškova za vaše Canopy sesije.
...
Uvezite podatke sesije iz JSON datoteke ili Canopy dijeljenog URL-a.

```bash
Canopy import <file>
```

Možete uvesti iz lokalne datoteke ili Canopy dijeljenog URL-a.
...
Pokrenite Canopy headless server sa web interfejsom.

```bash
Canopy web
```

Ovo pokreće HTTP server i otvara web pretraživač za pristup Canopy-u preko web interfejsa. Postavite `OPENCODE_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `Canopy`).
...
Deinstalirajte Canopy i uklonite sve povezane datoteke.
...
Ažurira Canopy na najnoviju verziju ili određenu verziju.
...
Canopy CLI prihvata sljedeće globalne zastavice.
...
Canopy se može konfigurirati pomoću varijabli okruženja.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Poništi osnovno korisničko ime autentifikacije (zadano `Canopy`) |
