...
Agent skills omogucavaju Canopy da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
Canopy pretrazuje ove lokacije:

- Konfiguracija projekta: `.Canopy/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/Canopy/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, Canopy ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.Canopy/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/Canopy/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.Canopy/skills/git-release/SKILL.md` ovako:
...
Canopy navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `canopy.json.guard`:
...
**Za ugradene agente** (u `canopy.json.guard`):
