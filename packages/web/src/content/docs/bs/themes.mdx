...
U Canopy mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, Canopy koristi nasu `Canopy` temu.
...
Canopy dolazi sa vise ugradenih tema.
...
- Zele da Canopy odgovara izgledu njihovog terminala
...
Canopy podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/Canopy/themes/*.json` ili `$XDG_CONFIG_HOME/Canopy/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.Canopy/themes/*.json`
4. **Trenutni radni direktorij** - `./.Canopy/themes/*.json`
