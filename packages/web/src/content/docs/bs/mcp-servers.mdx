...
Možete dodati vanjske alate u Canopy koristeći _Model Context Protocol_, ili MCP. Canopy podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [Canopy Config](https://canopy.dev/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/Canopy`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
Canopy automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, Canopy će:
...
Ako server zahtijeva autentifikaciju, Canopy će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `canopy mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, Canopy sigurno čuva tokene u `~/.local/share/Canopy/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u Canopy, zajedno s ugrađenim alatima. Možete njima upravljati kroz Canopy konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete Canopy sa vašim Sentry nalogom.
