---
title: Web
description: Using Canopy in your browser.
---

Canopy can run as a web application in your browser, providing the same powerful AI coding experience without needing a terminal.

![Canopy Web - New Session](../../assets/web/web-homepage-new-session.png)
...
This starts a local server on `127.0.0.1` with a random available port and automatically opens Canopy in your default browser.
...
:::tip[Windows Users]
For the best experience, run `canopy web` from [WSL](/docs/windows-wsl) rather than PowerShell. This ensures proper file system access and terminal integration.
:::
...
By default, Canopy picks an available port. You can specify a port:
...
By default, the server binds to `127.0.0.1` (localhost only). To make Canopy accessible on your network:

```bash
opencode web --hostname 0.0.0.0
```

When using `0.0.0.0`, Canopy will display both local and network addresses:
...
Once started, the web interface provides access to your Canopy sessions.
