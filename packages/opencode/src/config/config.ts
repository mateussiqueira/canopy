...
import { httpClient } from "@canopystack/core/effect/layer-node-platform"
import { serviceUse } from "@canopystack/core/effect/service-use"
...
import { InstallationLocal, InstallationVersion } from "@canopystack/core/installation/version"
import { existsSync } from "fs"
import { Account } from "@/account/account"
import { isRecord } from "@/util/record"
import type { ConsoleState } from "@canopystack/core/v1/config/console-state"
...
import { RemoteAuthError } from "@canopystack/core/v1/config/error"
import { ConfigPermissionV1 } from "@canopystack/core/v1/config/permission"
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.CANOPY_CONFIG && !Flag.CANOPY_CONFIG_DIR && !Flag.CANOPY_CONFIG_CONTENT) {
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "CANOPY_CONFIG_CONTENT") return "local"
        ...
        if (Flag.CANOPY_CONFIG) {
          yield* merge(Flag.CANOPY_CONFIG, yield* loadFile(Flag.CANOPY_CONFIG, authEnv))
          yield* Effect.logDebug("loaded custom config", { path: Flag.CANOPY_CONFIG })
        }

        if (!Flag.CANOPY_DISABLE_PROJECT_CONFIG) {
        ...
        if (Flag.CANOPY_CONFIG_DIR) {
          yield* Effect.logDebug("loading config from CANOPY_CONFIG_DIR", { path: Flag.CANOPY_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".canopy") || dir === Flag.CANOPY_CONFIG_DIR) {
            for (const file of ["canopy.json", "canopy.jsonc"]) {
          ...
          // Auto-discovered plugins under `.canopy/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.CANOPY_CONFIG_CONTENT) {
          const source = "CANOPY_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.CANOPY_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          yield* Effect.logDebug("loaded custom config from CANOPY_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["CANOPY_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("CANOPY_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.CANOPY_PERMISSION) {
          try {
            result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.CANOPY_PERMISSION))
          } catch (err) {
            yield* Effect.logWarning("CANOPY_PERMISSION contains invalid JSON, skipping", { err })
          }
        }
        ...
        if (Flag.CANOPY_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.CANOPY_DISABLE_PRUNE) {
