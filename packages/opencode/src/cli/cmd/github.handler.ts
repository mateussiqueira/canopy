  ...
  yield* Effect.promise(async () => {
    {
      ...
      function printNextSteps() {
        ...
        prompts.outro(
          [
            ...
            "",
            "   Learn more about the GitHub agent - https://canopy.dev/docs/github/#usage-examples",
          ].join("\n"),
        )
      }
      ...
      async function installGitHubApp() {
        ...
        async function getInstallation() {
          return await fetch(`https://api.canopy.dev/get_github_app_installation?owner=${app.owner}&repo=${app.repo}`)
            .then((res) => res.json())
...
jobs:
  opencode:
    ...
    steps:
      ...
      - name: Run opencode
        uses: mateussiqueira/canopy/github@latest${envStr}
        with:
...
export const githubRun = Effect.fn("Cli.github.run")(function* (args: { event?: string; token?: string }) {
  ...
  yield* Effect.promise(async () => {
    ...
    const shareBaseUrl = isMock ? "https://dev.canopy.dev" : "https://canopy.dev"
    ...
    try {
      ...
      UI.println(`opencode session ${session.id}`)
    ...
    function normalizeOidcBaseUrl(): string {
      ...
      if (!value) return "https://api.canopy.dev"
