...
//   - CANOPY_CONFIG_CONTENT      : provider config inline, no files to find
//   - CANOPY_TEST_HOME           : pins os.homedir() → tmpdir
//   - CANOPY_DISABLE_PROJECT_CONFIG : skip walking up for opencode.json
//   - CANOPY_PURE                : skip external plugin discovery + install
//   - CANOPY_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    CANOPY_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    CANOPY_CONFIG_CONTENT: configJson,
    CANOPY_DISABLE_PROJECT_CONFIG: "1",
    CANOPY_PURE: "1",
    CANOPY_DISABLE_AUTOUPDATE: "1",
    CANOPY_DISABLE_AUTOCOMPACT: "1",
    CANOPY_DISABLE_MODELS_FETCH: "1",
    CANOPY_AUTH_CONTENT: "{}",
  }
}
  ...
  return Effect.gen(function* () {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          CANOPY_CONFIG_CONTENT: JSON.stringify({
