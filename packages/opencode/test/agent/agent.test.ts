...
import { LocationServiceMap } from "@canopystack/core/location-layer"
...
it.instance("plan agent denies edits except .canopy/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".canopy/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".canopy", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.CANOPY_TEST_HOME
      process.env.CANOPY_TEST_HOME = test.directory
