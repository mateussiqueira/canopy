...
describe("buildRequestParts", () => {
    ...
    expect(
      result.requestParts.some(
        (part) =>
          part.type === "text" &&
          part.synthetic &&
          part.metadata?.canopyComment &&
          (part.metadata.canopyComment as { comment?: string }).comment === "check this",
      ),
    ).toBe(true)
  ...
  test("preserves an external attachment source path for the model", () => {
      ...
      images: [
        {
          ...
          sourcePath: "C:\\Users\\Luke\\AppData\\Roaming\\ai.canopy.desktop.beta\\opencode.global.dat",
          mime: "text/plain",
    ...
    expect(result.requestParts.find((part) => part.type === "file")?.filename).toBe(
      "C:\\Users\\Luke\\AppData\\Roaming\\ai.canopy.desktop.beta\\opencode.global.dat",
    )
  })
