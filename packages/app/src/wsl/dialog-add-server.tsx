...
import { showToast } from "@canopystack/ui/toast"
...
export function DialogAddWslServer(props: DialogWslServerProps = {}) {
  ...
  const opencodeCheck = createMemo(() => {
    const distro = selectedDistro()
    if (!distro) return null
    return current()?.canopyChecks[distro] ?? null
  })
  ...
  const opencodeMessage = createMemo(() => {
    ...
    if (opencodeReady()) {
      return distro
        ? language.t("wsl.onboarding.canopyReadyIn", { distro })
        : language.t("wsl.onboarding.canopyReady")
    }
    return distro
        ...
        title:
          step === "wsl"
            ...
            : step === "distro"
              ...
              : language.t("wsl.onboarding.step.canopy"),
        state:
          active === step
            ? "current"
            : step === "wsl"
        ...
        >
          ...
          <Switch>
            ...
            <Match when={activeStep() === "opencode"}>
              <div class="rounded-md bg-surface-base p-4 flex flex-col gap-3">
                <div class="flex items-center justify-between gap-3">
                  <div class="text-14-medium text-text-strong">{language.t("wsl.onboarding.step.canopy")}</div>
                  <div class="flex items-center gap-2">
                    <Show when={selectedDistro()}>
                      <Button
                        variant="ghost"
