...
export async function spawnWslSidecar(
  ...
  if (!opencode) throw new Error(`Canopy is not installed in ${distro}`)
  ...
  const script = [
    ...
    "export PATH",
    "export WSLENV=",
    "export CANOPY_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export CANOPY_CLIENT=desktop",
    `export CANOPY_SERVER_USERNAME=${shellEscape(username)}`,
    `export CANOPY_SERVER_PASSWORD=${shellEscape(password)}`,
