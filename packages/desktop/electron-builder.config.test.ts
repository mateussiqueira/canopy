...
const channels = [
  { channel: "dev", appId: "ai.canopy.desktop.dev" },
  { channel: "beta", appId: "ai.canopy.desktop.beta" },
  { channel: "prod", appId: "ai.canopy.desktop" },
] as const

for (const channel of channels) {
  test(`uses one Linux desktop identity for ${channel.channel}`, async () => {
    const previous = process.env.CANOPY_CHANNEL
    process.env.CANOPY_CHANNEL = channel.channel

    const module = await import(`./electron-builder.config.ts?channel=${channel.channel}`)
    const config = module.default as Configuration

    if (previous === undefined) delete process.env.CANOPY_CHANNEL
    else process.env.CANOPY_CHANNEL = previous
...
test("keeps a hidden prod launcher for old Linux pins", async () => {
  const previous = process.env.CANOPY_CHANNEL
  process.env.CANOPY_CHANNEL = "prod"
